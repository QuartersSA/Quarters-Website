...
async function GET(request) {
  ...
  try {
    ...
    if (q) {
      conditions.push(`(LOWER(b.name) LIKE $${idx} OR LOWER(b.iban) LIKE $${idx} OR LOWER(COALESCE(b.bank_name,'')) LIKE $${idx})`);
...
async function POST(request) {
  ...
  try {
    ...
    const swift = body.swift ? String(body.swift).trim().toUpperCase() : null;
