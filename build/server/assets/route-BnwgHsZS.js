  ...
  try {
    ...
    if (items.length > 0 && Number.isFinite(grandTotal) && grandTotal > 0) {
      ...
      const effectiveSum = items.reduce((acc, item) => {
        const gross = Number(item.quantity) * Number(item.unit_price) * (item.amount_includes_tax ? 1 : 1 + (Number(item.tax_rate) || 0) / 100);
        return acc + gross;
      }, 0);
      if (effectiveSum > 0 && !close(effectiveSum, grandTotal) && grandTotal / effectiveSum > 0.5 && grandTotal / effectiveSum < 1.5) {
        const factor = grandTotal / effectiveSum;
        for (const item of items) {
          item.unit_price = Math.round(Number(item.unit_price) * factor * 10000) / 10000;
        }
        const diff = Math.round((effectiveSum - grandTotal) * 100) / 100;
        const note = diff > 0 ? `تم توزيع خصم ${diff} على أسعار الوحدات ليطابق مجموع البنود إجمالي الفاتورة.` : `تمت تسوية فرق ${Math.abs(diff)} على أسعار الوحدات ليطابق مجموع البنود إجمالي الفاتورة.`;
