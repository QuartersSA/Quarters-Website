...
async function GET(request) {
  try {
    ...
    const todayQuery = await sql`SELECT (NOW() AT TIME ZONE 'Asia/Riyadh')::date AS today`;
    const today = todayQuery[0]?.today;

    // 1. My tasks due today
    const myTasksToday = await sql(`SELECT
      ...
      ORDER BY
        ...
        END,
        t.id DESC
      LIMIT 50`, [today, employeeId]);

    // 2. My overdue tasks
    const myOverdueTasks = await sql(`SELECT
        t.id, t.title, t.status, t.priority, t.due_date, t.space_id,
        COALESCE(s.name, '') as space_name
      FROM workspace_tasks t
      LEFT JOIN workspace_spaces s ON s.id = t.space_id
      ...
      LIMIT 20`, [today, employeeId]);

    // 3. Unread messages count
    const unreadRows = await sql(`SELECT COALESCE(SUM(
      ...
      WHERE tm.employee_id = $1`, [employeeId]);
    ...
    const upcomingTasks = await sql(`SELECT
    ...
    const healthRows = await sql(`SELECT
        COUNT(*) as total_done,
    ...
    const healthPercent = totalDoneThisMonth > 0 ? Math.round(onTimeCount / totalDoneThisMonth * 100) : null;
    const healthScore = {
