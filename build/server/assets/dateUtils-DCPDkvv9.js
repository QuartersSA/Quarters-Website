...
function formatRiyadhDateTimeForInput(value = new Date()) {
 ...
 */
function riyadhDateKeyFromOffset(days = 0) {
  const n = Number(days);
 ...
 */
function riyadhDateKeyFromMonthOffset(months = 0) {
 ...
 */
function validateBusinessDate(value, options = {}) {
  const {
    allowFuture = 1,
    minYear = 2020,
    label = "التاريخ"
  ...
  if (d > maxFuture) {
    return {
      ok: false,
      error: `${label} في المستقبل غير مقبول`
    };
  }
  return {
    ok: true,
...
function parseBusinessTimestamp(value, options = {}) {
  const result = validateBusinessDate(value, options);
  return result.ok ? toDbTimestamp(result.date) : null;
}

export { LOCALE as L, formatRiyadhDateTimeForInput as a, riyadhMonthKeyFromOffset as b, currentRiyadhMonthKey as c, formatDateTime as d, formatDateForInput as e, formatRiyadhDateForInput as f, currentRiyadhHour as g, riyadhDateKeyFromMonthOffset as h, formatTime as i, formatDate as j, parseBusinessTimestamp as p, riyadhDateKeyFromOffset as r, todayRiyadhDateKey as t, validateBusinessDate as v };
