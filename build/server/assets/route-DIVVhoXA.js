...
import { r as requireWorkspaceEmployee } from './_utils-CDFiiX1V.js';
...
async function PATCH(request) {
  try {
    ...
    // Get current task
    const rows = await sql`SELECT id, status, title FROM workspace_tasks WHERE id = ${taskId}`;
    const task = rows[0];
    if (!task) {
      return Response.json({
    ...
    await sql("INSERT INTO workspace_task_events (task_id, event_type, actor_employee_id, summary, meta) VALUES ($1, $2, $3, $4, $5::jsonb)", [taskId, "updated", employeeId, "تم تغيير حالة المهمة", meta]);
    ...
    const updated = await sql`SELECT * FROM workspace_tasks WHERE id = ${taskId}`;
    return Response.json({
      task: updated[0],
      changed: true
    });
