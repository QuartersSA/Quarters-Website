...
const ocrPdfDetails = async (file, onProgress) => {
  try {
    ensureWorker();
    const data = await file.arrayBuffer();
    const pdf = await pdfjs.getDocument({
    ...
    try {
      for (let pageNumber = 1; pageNumber <= pdf.numPages; pageNumber++) {
        ...
        text += (result.text || "") + "\n";
        const lines = (result.lines || []).map(line => String(line.text || "").replace(/\s+/g, " ").trim()).filter(Boolean);
        // Older tesseract builds may omit .lines — fall back to text rows.
        pageLines.push(lines.length > 0 ? lines : String(result.text || "").split("\n").map(line => line.trim()).filter(Boolean));
      }
    } finally {
...
export { extractPdfDetails, extractTextFromPDF, ocrPdfDetails };
