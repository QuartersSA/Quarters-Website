...
async function POST(request, {
  ...
  try {
    ...
    const existing = await sql`SELECT id, status FROM upload_sessions WHERE id = ${id}`;
    if (!existing?.length) {
      return Response.json({
    ...
    await sql("INSERT INTO upload_chunks (session_id, chunk_index, data) VALUES ($1, $2, $3) ON CONFLICT (session_id, chunk_index) DO UPDATE SET data = EXCLUDED.data", [id, chunkIndex, buf]);
    return Response.json({
      ok: true
    }, {
      status: 200
