...
async function POST(request) {
  ...
  try {
    ...
    const ids = [...new Set((Array.isArray(body.invoice_ids) ? body.invoice_ids : []).map(Number).filter(id => Number.isInteger(id) && id > 0))];
    if (ids.length === 0) {
      return Response.json({
        error: "حدد فاتورة واحدة على الأقل"
      }, {
        status: 400
      });
    }
    ...
    const supplierKey = inv => inv.contact_id ? `c:${inv.contact_id}` : `s:${String(inv.supplier || "").trim().toLowerCase()}`;
    ...
    const currencies = new Set(invoices.map(inv => (inv.currency || "SAR").toUpperCase()));
    ...
    const rows = invoices.map(inv => ({
      ...inv,
      balance: round2(Math.max(Number(inv.total_amount || 0) - Number(inv.paid_amount || 0), 0))
    }));
    ...
    const paymentNote = [`دفعة جماعية (${rows.length} فاتورة): ${numbers}`, extraNotes].filter(Boolean).join(" — ");
    ...
    for (const [creatorId, creatorRows] of byCreator) {
      const lines = [`💳 دفعة جماعية سدّدت ${creatorRows.length} من فواتيرك`, rows[0].supplier ? `المورد: ${rows[0].supplier}` : null, ...creatorRows.map(inv => `• ${inv.invoice_number}: ${inv.balance.toFixed(2)} ${currency}`), `التاريخ: ${paymentDate}`, bankName ? `البنك: ${bankName}` : null, actorName ? `سجلها: ${actorName}` : null, receiptUrl ? `الإيصال: ${receiptUrl}` : null].filter(Boolean);
    ...
    return Response.json({
      error: "فشل تسجيل الدفعة الجماعية",
