...
const BENEFICIARY_TYPES = new Set(["bank", "government"]);
...
async function GET(request) {
  ...
  try {
    ...
    if (q) {
      conditions.push(`(LOWER(b.name) LIKE $${idx} OR LOWER(b.iban) LIKE $${idx} OR LOWER(COALESCE(b.account_number,'')) LIKE $${idx} OR LOWER(COALESCE(b.bank_name,'')) LIKE $${idx})`);
...
async function POST(request) {
  ...
  try {
    ...
    const beneficiaryType = BENEFICIARY_TYPES.has(body.beneficiary_type) ? body.beneficiary_type : "bank";
    const isGovernment = beneficiaryType === "government";
    const iban = body.iban ? String(body.iban).replace(/\s+/g, "").toUpperCase() : "";
    const accountNumber = body.account_number ? String(body.account_number).replace(/\s+/g, "") : null;
    ...
    const currency = isGovernment ? "SAR" : body.currency ? String(body.currency).trim().toUpperCase() : "SAR";
    const bankName = isGovernment ? null : body.bank_name ? String(body.bank_name).trim() : null;
    const swift = isGovernment ? null : body.swift ? String(body.swift).trim().toUpperCase() : null;
    const contactId = body.contact_id === null || body.contact_id === undefined || body.contact_id === "" ? null : Number(body.contact_id);
    if (body.contact_id !== null && body.contact_id !== undefined && body.contact_id !== "" && (!Number.isFinite(contactId) || contactId <= 0)) {
      return Response.json({
...
export { BENEFICIARY_TYPES, GET, POST };
