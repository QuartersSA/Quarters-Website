 ...
 */
async function findDisabledItemsAtBranch(branchId, itemIds) {
  const bid = Number(branchId);
  if (!Number.isFinite(bid) || bid <= 0) return [];
  if (!Array.isArray(itemIds) || itemIds.length === 0) return [];
  const normalised = itemIds.map(x => Number(x)).filter(x => Number.isFinite(x) && x > 0);
  if (normalised.length === 0) return [];
  const rows = await sql(`SELECT item_id
       FROM item_branch_disabled
      WHERE branch_id = $1
        AND item_id   = ANY($2::int[])`, [bid, normalised]);
 ...
 */
async function assertItemsEnabledAtBranch(branchId, itemIds, lookupName) {
  ...
  return {
    status: 400,
    body: {
      error: "بعض الأصناف معطّلة في هذا الفرع. أعد تفعيلها من إدارة الفروع قبل المتابعة.",
...
export { assertItemsEnabledAtBranch as a, findDisabledItemsAtBranch as f };
