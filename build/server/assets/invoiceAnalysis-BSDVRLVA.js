...
const OUTPUT_SCHEMA = {
  ...
  required: ["document_type", "invoice_number", "invoice_date", "due_date", "supplier_name", "supplier_vat_number", "contact_id", "contact_matched_by", "currency", "total", "discount", "tax", "items", "operator_note"],
  properties: {
    document_type: {
      type: "string",
      enum: ["quote", "tax_invoice", "payment_receipt", "other"],
      description: "Document classification: quote = عرض سعر/Quotation; tax_invoice = فاتورة ضريبية (incl. simplified); payment_receipt = سند سداد/إيصال دفع/إشعار تحويل; other = anything else"
    },
    invoice_number: {
      type: ["string", "null"]
    },
    ...
    items: {
      ...
      items: {
        ...
        properties: {
          ...
          pack_size_kg: {
            type: ["number", "null"],
            description: "For coffee bean lines only: kilograms per sack/bag as printed (e.g. '60 كجم', '69 KG', '1x60kg'). Null when not printed or not a coffee line."
          },
          quantity_unit: {
            type: ["string", "null"],
            description: "For coffee bean lines only: 'sack' when quantity counts sacks/bags, 'kg' when quantity is kilograms. Null otherwise."
          }
        }
      }
    },
...
async function runInvoiceAnalysis({
  ...
  for (const item of analysis.items || []) {
    ...
    item.pack_size_kg = Number.isFinite(pack) && pack >= 5 && pack <= 1000 ? Math.round(pack * 1000) / 1000 : null;
...
export { FILE_MEDIA_TYPES as F, runInvoiceAnalysis as r };
