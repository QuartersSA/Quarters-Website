...
import { s as sendWhatsAppViaWasender } from './wasender-DEtMgWCV.js';
import { n as notifyByPref, a as notifyLowStockIfAny } from './waNotify-BypZ6oBy.js';
import { f as findDisabledItemsAtBranch } from './branchVisibility-DozON22O.js';
...
async function POST(request) {
  ...
  try {
    ...
    const itemUnitById = new Map(itemIds.map(id => [id, snapshotForItem(unitSnapshots, id).unitName]));
    ...
    {
      const [disabledAtFrom, disabledAtTo] = await Promise.all([findDisabledItemsAtBranch(fromId, itemIds), findDisabledItemsAtBranch(toId, itemIds)]);
      const combined = new Set([...disabledAtFrom, ...disabledAtTo]);
      if (combined.size > 0) {
        const offendingIds = Array.from(combined);
        const labels = offendingIds.map(id => itemNameById.get(id) || `#${id}`);
        return Response.json({
          error: "بعض الأصناف معطّلة في فرع المرسل أو المستقبل. أعد تفعيلها من إدارة الفروع قبل التحويل.",
    ...
    const [opPair] = await sql(`
      ...
      `, [transferNumber, fromId, actingEmployeeId, toId, note || null, parsedDate, perItemIds, fromNewArr, toNewArr, cleanedItems.map(it => Number(it.quantity) || 0), unitIdArr, unitNameArr, unitFactorArr]);
    const opOut = {
      id: opPair.out_id,
