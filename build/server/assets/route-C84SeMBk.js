...
const WASTE_REASONS = new Set(["expiry",
// تاريخ صلاحية
"customer_return",
// إرجاع من العميل (note = رقم الفاتورة)
"order_error",
// خطأ في الطلب
...
async function POST(request) {
  ...
  try {
    ...
    }).filter(it => Number.isFinite(it.itemId) && it.itemId > 0 && Number.isFinite(it.quantity) && it.quantity > 0);
    if (clean.length === 0) {
      return Response.json({
    ...
    const itemRows = await sql(`SELECT id, name, COALESCE(base_purchase_cost, cost) AS unit_cost
       FROM items WHERE id = ANY($1::bigint[])`, [itemIds]);
...
async function GET(request) {
  ...
  try {
    ...
    const bFilter = branchId && Number.isFinite(Number(branchId)) && Number(branchId) > 0 ? Number(branchId) : null;
    const reasonFilter = searchParams.get("reason");
    const rFilter = reasonFilter && WASTE_REASONS.has(reasonFilter) ? reasonFilter : null;
    const ops = await sql(`
        SELECT
          o.id,
          o.branch_id,
    ...
    const topItems = await sql(`
        ...
        LIMIT 10
      `, [bFilter, from || null, to || null]);
    const grandTotal = ops.reduce((s, o) => s + (Number(o.total_cost) || 0), 0);
    const totalLines = ops.reduce((s, o) => s + (Number(o.items_count) || 0), 0);
    return Response.json({
      operations: ops,
