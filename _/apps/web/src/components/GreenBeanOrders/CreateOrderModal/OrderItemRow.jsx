...
export function OrderItemRow({
  ...
  return (
    <div className="rounded-3xl border border-white/10 overflow-hidden bg-white/[0.02]">
      <div className="px-4 py-3 flex items-center justify-between gap-2 bg-white/[0.03]">
        <div className="text-white/80 font-semibold">صنف #{idx + 1}</div>
        <button
          type="button"
      ...
      <div className="p-4 grid grid-cols-1 md:grid-cols-2 gap-3">
        ...
        <div className="md:col-span-2 grid grid-cols-1 md:grid-cols-3 gap-3">
          <div className={`${ws.glass} ${ws.card} p-3`}>
            <div className="text-xs text-white/50">الإجمالي (شامل الضريبة)</div>
            <div className="text-white font-bold mt-1">
              {formatMoney(computed.totalIncl)}
            </div>
          </div>
          <div className={`${ws.glass} ${ws.card} p-3`}>
            <div className="text-xs text-white/50">الواصل بعد الهدر</div>
            <div className="text-white font-bold mt-1">
              {formatQty(computed.receivedAfterWaste)}
            </div>
          </div>
          <div className={`${ws.glass} ${ws.card} p-3`}>
            <div className="text-xs text-white/50">سعر الكيلو الصافي</div>
