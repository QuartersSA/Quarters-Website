...
import {
  ...
  notifyTaskCreatorOnTaskWhatsApp,
...
function normalizeAttachments(input) {
  ...
  for (const raw of arr) {
    ...
    const mimeType =
      raw.mimeType === null || raw.mimeType === undefined
        ? null
        : String(raw.mimeType).trim() || null;

    const name =
      raw.name === null || raw.name === undefined
        ? null
        : String(raw.name).trim() || null;
    ...
    out.push({ url, mimeType, name, sizeBytes });
...
function buildDiff(before, after) {
  ...
  if (!same(beforeTags, afterTags)) {
    diff.tags = { from: beforeTags, to: afterTags };
  }
...
function summaryFromDiff(diff) {
  if (diff.closed_not_completed) {
    return diff.closed_not_completed.to
      ? "تم إغلاق المهمة لعدم إتمامها"
      : "تم إلغاء إغلاق المهمة";
  }
...
function withTimeout(promise, ms) {
  ...
  const timeoutPromise = new Promise((resolve) => {
    timeoutId = setTimeout(
      () => resolve({ ok: true, skipped: true, reason: "timeout" }),
      ms,
    );
  });
...
export async function PATCH(request, { params }) {
  try {
    ...
    const beforeIsSoftDeleted =
      Array.isArray(beforeTask.tags) &&
      beforeTask.tags.includes(SYSTEM_DELETED_TAG);
    ...
    if (closeNotCompletedProvided) {
      ...
      if (tagsToWrite === undefined) {
        tagsToWrite = Array.isArray(beforeTask.tags)
          ? [...beforeTask.tags]
          : [];
      }
      ...
      const withoutClosed = baseArr.filter(
        (t) => String(t) !== SYSTEM_CLOSED_TAG,
      );
      const nextArr = closeNotCompleted
        ? [...withoutClosed, SYSTEM_CLOSED_TAG]
        : withoutClosed;
    ...
    if (!attachmentsProvided && body.imageUrl !== undefined) {
      ...
      const imageUrl =
        imageUrlRaw === null || imageUrlRaw === undefined
          ? null
          : String(imageUrlRaw).trim() || null;

      const imageMimeType =
        imageMimeTypeRaw === null || imageMimeTypeRaw === undefined
          ? null
          : String(imageMimeTypeRaw).trim() || null;

      const imageName =
        imageNameRaw === null || imageNameRaw === undefined
          ? null
          : String(imageNameRaw).trim() || null;
    ...
    if (JSON.stringify(beforeAssignees) !== JSON.stringify(afterAssignees)) {
      diff.assignees = { from: beforeAssignees, to: afterAssignees };
    }
    ...
    if (Object.keys(diff).length > 0) {
      ...
      const meta = {
        diff,
        before: { assignee_ids: beforeAssignees },
        after: { assignee_ids: afterAssignees },
      };

      await sql(
        "INSERT INTO workspace_task_events (task_id, event_type, actor_employee_id, summary, meta) VALUES ($1, $2, $3, $4, $5::jsonb)",
        [id, "updated", employeeId, summary, JSON.stringify(meta)],
      );
      ...
      const assigneesText = assigneeNames.length
        ? assigneeNames.join("، ")
        : "—";
      ...
      const lines = [
        "تحديث مهمة",
        formatTaskLine("العنوان", afterTask?.title || ""),
        formatTaskLine("المساحة", spaceName),
        formatTaskLine("نوع التحديث", summary),
        formatTaskLine("المكلّف", assigneesText),
        formatTaskLine("بواسطة", actorName),
        formatTaskLine("رقم المهمة", `#${id}`),
      ];
      ...
      const creatorMessage = message.replace(
        "تحديث مهمة",
        "تحديث على مهمة أنشأتها",
      );

      // 1) notify assignees (exclude actor + exclude creator to prevent duplicate)
      const assigneesWithoutCreator = Array.isArray(afterAssignees)
        ? afterAssignees.filter((x) => String(x) !== String(creatorEmployeeId))
        : [];
    ...
    return Response.json({ task: afterTask }, { status: 200 });
  } catch (error) {
    ...
    return Response.json(
      details
        ? { error: "فشل تحديث المهمة", details }
...
function diffDetailsLines(diff) {
  ...
  if (diff.status) {
    lines.push(
      `- الحالة: ${diff.status.from ?? "—"} → ${diff.status.to ?? "—"}`,
    );
  }
  if (diff.priority) {
    lines.push(
      `- الأولوية: ${diff.priority.from ?? "—"} → ${diff.priority.to ?? "—"}`,
    );
  }
  if (diff.due_date) {
  ...
  if (diff.title) {
    lines.push(
      `- العنوان: ${diff.title.from ?? "—"} → ${diff.title.to ?? "—"}`,
    );
  }
  if (diff.space_id) {
    lines.push(
      `- المساحة: ${diff.space_id.from ?? "—"} → ${diff.space_id.to ?? "—"}`,
    );
  }
  if (diff.assignees) {
    const from = Array.isArray(diff.assignees.from)
      ? diff.assignees.from.join(",")
      : "—";
    const to = Array.isArray(diff.assignees.to)
      ? diff.assignees.to.join(",")
      : "—";
...
export async function DELETE(request, { params }) {
  try {
    ...
    await sql(
      "INSERT INTO workspace_task_events (task_id, event_type, actor_employee_id, summary, meta) VALUES ($1, $2, $3, $4, $5::jsonb)",
      [id, "deleted", employeeId, "تم حذف المهمة", JSON.stringify(deleteMeta)],
    );
