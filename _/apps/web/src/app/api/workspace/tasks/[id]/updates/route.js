...
export async function GET(request, { params }) {
  try {
    ...
    return Response.json({ updates: fixed });
  } catch (error) {
...
export async function POST(request, { params }) {
  try {
    ...
    if (!text) {
      return Response.json({ error: "نص التحديث مطلوب" }, { status: 400 });
    }
    ...
    if (update?.id) {
      ...
      await sql(
        ...
        [
          ...
          "updated",
          employeeId,
          update.image_url ? "تمت إضافة تحديث مع مرفق" : "تمت إضافة تحديث",
          JSON.stringify(meta),
        ],
      );
      ...
      const creatorMessage = message.replace(
        "ملاحظة/تحديث جديد على المهمة",
        "ملاحظة/تحديث جديد على مهمة أنشأتها",
      );

      // 1) notify assignees (exclude creator to prevent duplicate)
      const assigneesWithoutCreator = Array.isArray(assigneeIds)
        ? assigneeIds.filter((x) => String(x) !== String(creatorEmployeeId))
        : [];
    ...
    return Response.json(
      details
        ? { error: "فشل إضافة تحديث للمهمة", details }
