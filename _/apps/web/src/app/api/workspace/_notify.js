...
export async function getTaskAssigneeEmployeeIds(taskId) {
...
export async function getEmployeeNames(employeeIds) {
...
export function buildTaskLinkLine(taskId) {
...
export function formatTaskLine(label, value) {
  const v =
...
export function clipText(input, maxLen = 600) {
...
export async function notifyAssigneesOnTaskWhatsApp({
  taskId,
  assigneeEmployeeIds,
  excludeEmployeeId,
  message,
  ...
  try {
    ...
    const allIds = Array.from(
      new Set((assigneeEmployeeIds || []).map(toInt).filter(Boolean)),
    );
    ...
    if (ids.length === 0) {
      return { ok: true, skipped: true, reason: "no_assignees" };
    }
    ...
    const results = await Promise.all(
      employees.map(async (emp) => {
        const r = await sendWhatsAppViaWasender({ to: emp.phone, text });
        if (!r.ok) {
        ...
        return { employeeId: emp.id, ok: r.ok };
      }),
    );
...
export async function notifyTaskCreatorOnTaskWhatsApp({
  taskId,
  creatorEmployeeId,
  message,
}) {
  try {
    ...
    if (!creatorId) {
      return { ok: true, skipped: true, reason: "no_creator" };
    }
