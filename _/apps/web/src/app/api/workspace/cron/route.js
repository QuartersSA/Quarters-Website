...
// A simple cron endpoint that triggers the reminders system.
// Call this every 1–4 hours from an external cron service (e.g. cron-job.org).
...
async function fetchEmployeeNameMap(employeeIds) {
  const ids = Array.from(
    new Set((employeeIds || []).map(toInt).filter(Boolean)),
  );
...
function buildDueTomorrowMessage(task, assigneesText) {
  const lines = [
    "تذكير: باقي يوم واحد على الاستحقاق",
    formatTaskLine("العنوان", task.title),
  ];
  if (task.parent_title) {
    lines.push(formatTaskLine("المهمة الرئيسية", task.parent_title));
  }
  lines.push(
    formatTaskLine("المساحة", spaceName(task.space_name)),
    formatTaskLine("الحالة", task.status),
    formatTaskLine("الأولوية", task.priority),
    formatTaskLine(
      "الاستحقاق",
      task.due_date ? formatDateOnly(task.due_date) : "—",
    ),
    formatTaskLine("المكلّف", assigneesText),
    formatTaskLine("رقم المهمة", `#${task.id}`),
    "سيتم إرسال تذكير كل 4 ساعات حتى يوم الاستحقاق.",
  );
...
function buildDueTodayMessage(task, assigneesText) {
  const lines = [
    "تذكير عاجل: استحقاق المهمة اليوم",
    "ملاحظة: هذه المهمة ضمن قائمة (استحقاق اليوم).",
    "الرجاء سرعة تنفيذ المهمة.",
    formatTaskLine("العنوان", task.title),
  ];
  if (task.parent_title) {
    lines.push(formatTaskLine("المهمة الرئيسية", task.parent_title));
  ...
  lines.push(
    ...
    formatTaskLine("المكلّف", assigneesText),
    formatTaskLine("رقم المهمة", `#${task.id}`),
    "سيتم إرسال تذكير كل 4 ساعات حتى نهاية اليوم.",
    "إذا لم يتم تنفيذها سيتم تسجيلها متأخرة وإرسال تنبيه الخصم.",
  );
  return `${lines.join("\n")}${buildTaskLinkLine(task.id)}`.trim();
}
...
function buildOverdueMessage(task, assigneesText) {
  ...
  lines.push(
    ...
    formatTaskLine("المكلّف", assigneesText),
    formatTaskLine("رقم المهمة", `#${task.id}`),
    "تم تسجيل المهمة كمتأخرة وسيتم خصم من الراتب.",
  );
  return `${lines.join("\n")}${buildTaskLinkLine(task.id)}`.trim();
}

async function runReminders() {
  // 1) Mark newly overdue tasks
  await sql`
    WITH vars AS (
  ...
  // 2) Due soon tasks (due today or tomorrow)
  ...
  for (const t of dueSoonTasks) {
    ...
    const message = t.is_due_today
      ? buildDueTodayMessage(t, assigneesText)
      : buildDueTomorrowMessage(t, assigneesText);
  ...
  if (dueSoonSentTaskIds.length > 0) {
    await sql(
      "UPDATE workspace_tasks SET due_soon_started_at = COALESCE(due_soon_started_at, NOW()), due_soon_last_notified_at = NOW() WHERE id = ANY($1::int[])",
      [dueSoonSentTaskIds],
    );
  }

  // 3) Overdue notifications (once per task)
  ...
  if (overdueSentTaskIds.length > 0) {
    await sql(
      "UPDATE workspace_tasks SET overdue_notified_at = NOW() WHERE id = ANY($1::int[])",
      [overdueSentTaskIds],
    );
  }
...
export async function GET(request) {
  try {
    ...
    return Response.json(
      { ok: false, error: "cron_failed", message: error.message },
      { status: 500 },
    );
  }
}
