...
export async function DELETE(request, { params }) {
  ...
  try {
    const [row] = await sql(
      `DELETE FROM accounting_green_bean_orders WHERE id = $1 RETURNING id`,
      [id],
    );
    ...
    return Response.json({ error: "فشل حذف الطلب" }, { status: 500 });
  }
}
...
export async function PUT(request, { params }) {
  ...
  try {
    ...
    const [existing] = await sql(
      `SELECT id FROM accounting_green_bean_orders WHERE id = $1 LIMIT 1`,
      [id],
    );
    if (!existing) {
    ...
    if (!orderDate) {
      return Response.json({ error: "اختر تاريخ الطلب" }, { status: 400 });
    }
    ...
    if (items.length === 0) {
      return Response.json({ error: "اختر البن في الصفوف" }, { status: 400 });
    }
    ...
    for (const it of items) {
      ...
      const baseChecks = [
        validateNonNegative(it.priceKgExclTax, "سعر الكيلو"),
        validateNonNegative(it.bagSizeKg, "حجم الخيشة"),
        validateNonNegative(roast, "تكلفة التحميص"),
        validateNonNegative(extra, "تكلفة إضافية للكيلو"),
      ];

      const baseFailed = baseChecks.find((c) => !c.ok);
      if (baseFailed) {
        return Response.json({ error: baseFailed.error }, { status: 400 });
      }

      if (it.receivedAfterWasteKg !== null) {
        const receivedCheck = validateNonNegative(
          it.receivedAfterWasteKg,
          "الكمية الواصلة",
        );
        if (!receivedCheck.ok) {
          return Response.json({ error: receivedCheck.error }, { status: 400 });
        }
      } else {
        const waste = it.wastePercent === null ? 0 : it.wastePercent;
        const wasteCheck = validateNonNegative(waste, "نسبة الهدر");
        if (!wasteCheck.ok) {
          return Response.json({ error: wasteCheck.error }, { status: 400 });
        }
      }
    }
    ...
    const beans = await sql(
      `SELECT id, name FROM accounting_green_beans WHERE id = ANY($1::bigint[])`,
      [beanIds],
    );
    ...
    // Update order header
    await sql(
      `UPDATE accounting_green_bean_orders
       SET order_date = $2, supplier_name = $3, note = $4, updated_at = CURRENT_TIMESTAMP
       WHERE id = $1`,
      [id, orderDate, supplierName, note],
    );

    // Delete old items
    await sql(
      `DELETE FROM accounting_green_bean_order_items WHERE order_id = $1`,
      [id],
    );
    ...
    try {
      const results = await Promise.all(
        items.map((it) => {
          const roastPerKgIncl =
            it.roastCostInclTax === null ? 8.05 : it.roastCostInclTax;
          const extraPerKg = it.extraCostPerKg === null ? 0 : it.extraCostPerKg;
          const effectiveExtraKg =
            it.extraCostKg !== null ? it.extraCostKg : Number(it.bagSizeKg);
          ...
          if (receivedAfterWasteKg !== null) {
            const rawWaste =
              bag > 0 ? (1 - receivedAfterWasteKg / bag) * 100 : 0;
    ...
    try {
      for (const item of insertedItems) {
        ...
        if (beanId && finalPrice != null) {
          await sql(
            `UPDATE items SET cost = $1 WHERE linked_green_bean_id = $2`,
            [finalPrice, beanId],
          );
        }
      }
      ...
      console.error(
        "Failed to auto-update linked item costs:",
        costUpdateError,
      );
    }
    ...
    return Response.json({ error: "فشل تعديل طلب البن" }, { status: 500 });
  }
}
