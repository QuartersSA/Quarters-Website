...
export async function GET(request) {
  ...
  try {
    ...
    return Response.json({ beans: rows || [] });
  } catch (error) {
    console.error("green beans GET error", error);
    return Response.json({ error: "فشل تحميل البن الأخضر" }, { status: 500 });
  }
}
...
export async function POST(request) {
  ...
  try {
    ...
    if (!row) {
      return Response.json({ error: "هذا الاسم موجود مسبقًا" }, { status: 409 });
    }

    return Response.json({ bean: row }, { status: 201 });
  } catch (error) {
    console.error("green beans POST error", error);
    return Response.json({ error: "فشل إضافة البن الأخضر" }, { status: 500 });
  }
}
