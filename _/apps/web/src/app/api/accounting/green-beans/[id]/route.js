...
export async function PUT(request, { params }) {
  ...
  try {
    ...
    const checks = [
      validateNonNegative(priceKgExclTax, "سعر الكيلو"),
      validateNonNegative(bagSizeKg, "حجم الخيشة"),
      validateNonNegative(roastCostExclTax, "تكلفة التحميص (غير شامل)"),
      validateNonNegative(roastCostInclTax, "تكلفة التحميص (شامل)"),
      validateNonNegative(extraCostPerKg, "التكاليف الإضافية للكيلو"),
      validateNonNegative(wastePercent, "نسبة الهدر"),
    ];

    const failed = checks.find((c) => !c.ok);
    if (failed) {
      return Response.json({ error: failed.error }, { status: 400 });
    }
    ...
    if (setClauses.length === 0) {
      const [existing] = await sql(
        `SELECT * FROM accounting_green_beans WHERE id = $1 LIMIT 1`,
        [id],
      );
      return Response.json({ bean: existing || null });
    }
    ...
    return Response.json({ bean: row });
  } catch (error) {
    console.error("green beans PUT error", error);
    return Response.json({ error: "فشل تحديث بيانات البن" }, { status: 500 });
  }
}
...
export async function GET(request, { params }) {
  ...
  try {
    ...
    return Response.json({ error: "فشل تحميل البن" }, { status: 500 });
  }
}
