...
export async function PATCH(request) {
  ...
  try {
    ...
    const [entry] = await sql(
      "SELECT e.id, e.run_id, e.net_salary, r.is_closed FROM accounting_payroll_entries e JOIN accounting_payroll_runs r ON r.id = e.run_id WHERE e.id = $1",
      [entryId],
    );
    ...
    const [updated] = await sql(
      `UPDATE accounting_payroll_entries
       SET is_paid = $1,
           paid_amount = $2,
           payment_method = $3,
           payment_note = $4,
    ...
    return Response.json(
      { error: "فشل تحديث حالة الدفع", details: error.message },
      { status: 500 },
    );
  }
}
