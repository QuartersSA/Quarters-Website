...
export default function AccountingDashboardPage() {
  ...
  if (shiftClosingsQuery.isLoading) {
    shiftHighlightsBody = <div className="text-slate-600 dark:text-white/60">جاري التحميل…</div>;
  } else if (shiftClosingsQuery.error) {
    shiftHighlightsBody = (
      <div className="text-red-700 dark:text-red-300">{shiftClosingsErrorText}</div>
    );
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between gap-2 flex-wrap">
          ...
          <div className="flex items-center gap-2">
            <span className="text-slate-900 dark:text-white font-extrabold">{totalDiffText}</span>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
          <div className={`${ws.glass} ${ws.card} p-3`}>
            <div className="text-xs text-slate-500 dark:text-white/50">فرق الكاش</div>
            <div className="text-slate-900 dark:text-white font-bold mt-1">{cashDiffText}</div>
          </div>
          <div className={`${ws.glass} ${ws.card} p-3`}>
            <div className="text-xs text-slate-500 dark:text-white/50">فرق الشبكة</div>
            <div className="text-slate-900 dark:text-white font-bold mt-1">{cardDiffText}</div>
          </div>
          <div className={`${ws.glass} ${ws.card} p-3`}>
            <div className="text-xs text-slate-500 dark:text-white/50">الإجمالي</div>
  ...
  if (payrollRunsQuery.isLoading) {
    payrollHighlightsBody = <div className="text-slate-600 dark:text-white/60">جاري التحميل…</div>;
  } else if (payrollRunsQuery.error) {
    payrollHighlightsBody = (
      <div className="text-red-700 dark:text-red-300">{payrollRunsErrorText}</div>
    );
  } else if (!latestRun) {
    payrollHighlightsBody = (
      <div className="text-slate-600 dark:text-white/60">لا يوجد مسير رواتب بعد.</div>
    );
  } else {
    payrollHighlightsBody = (
      <div className="space-y-2">
        <div className="flex items-center justify-between gap-2 flex-wrap">
          <div className="text-slate-800 dark:text-white/80">
            <span className="text-slate-500 dark:text-white/40">الشهر: </span>
            <span className="font-semibold text-slate-900 dark:text-white">
              {latestRunMonthText}
            </span>
          </div>
          <div className="text-xs text-slate-500 dark:text-white/50">{latestRunCreatedAtText}</div>
        </div>

        <div className={`${ws.glass} ${ws.card} p-3`}>
          <div className="text-xs text-slate-500 dark:text-white/50">آخر من أنشأه</div>
        ...
        <div className="px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-2 min-w-0">
            <div className={`${ws.iconBox} w-10 h-10`}>
              <LayoutDashboard className="w-5 h-5 text-emerald-700 dark:text-emerald-200" />
            </div>
        ...
        <div className="mx-auto w-full max-w-[1200px] space-y-5">
          ...
          <div className="hidden lg:flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-slate-600 dark:text-white/55 mt-1">لوحة المحاسبة</div>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {cards.map((c) => {
              const Icon = c.Icon;
              const arrowClass =
                "w-5 h-5 text-slate-500 dark:text-white/40 group-hover:text-slate-700 dark:group-hover:text-white/70 transition-colors";
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    <div className={`${ws.iconBox} !bg-emerald-500/10`}>
                      <Icon className="w-6 h-6 text-emerald-700 dark:text-emerald-200" />
                    </div>
                  ...
                  <div className="text-slate-600 dark:text-white/55 text-sm mt-1">{c.subtitle}</div>
                </a>
              );
            })}
            ...
            >
              <div className="flex items-start justify-between gap-3">
                <div className={`${ws.iconBox} !bg-emerald-500/10`}>
                  <TrendingUp className="w-6 h-6 text-emerald-700 dark:text-emerald-200" />
                </div>
              </div>
              <div className="mt-4 text-slate-900 dark:text-white font-bold text-lg tracking-tight">
                قريبًا
