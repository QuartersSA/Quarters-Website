...
export async function POST(request) {
  ...
  if (!category) return Response.json({ error: "التصنيف مطلوب" }, { status: 400 });
  if (!name_ar) return Response.json({ error: "الاسم بالعربي مطلوب" }, { status: 400 });
  ...
  if (price !== null && (!Number.isFinite(price) || price < 0)) {
    return Response.json({ error: "السعر غير صحيح" }, { status: 400 });
  }
...
export async function PUT(request) {
  ...
  if (b.category !== undefined) {
    ...
    if (!v) return Response.json({ error: "التصنيف لا يمكن أن يكون فارغاً" }, { status: 400 });
    push("category", v);
  }
  ...
  if (b.sort_order !== undefined) {
    ...
    if (!Number.isFinite(v)) {
      return Response.json({ error: "ترتيب غير صحيح" }, { status: 400 });
    }
    push("sort_order", v);
  }
