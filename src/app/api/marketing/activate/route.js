...
export async function POST(request) {
  ...
  if (!slug) return Response.json({ error: "الكود مطلوب" }, { status: 400 });
  if (!pin) return Response.json({ error: "رقم التفعيل مطلوب" }, { status: 400 });
  ...
  if (row.state === "active") {
    return Response.json(
      {
        error:
          "هذا الكود مُفعَّل مسبقاً ولا يمكن استخدامه مرة أخرى.",
        blogger: row,
      },
      { status: 409 },
    );
  ...
  return Response.json({ ok: true, blogger: updated });
}
