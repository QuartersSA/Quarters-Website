...
export async function PUT(request, { params: { id } }) {
  ...
  try {
    ...
    const bonusDate =
      body.bonus_date !== undefined || body.bonusDate !== undefined
        ? normalizeIsoDate(body.bonus_date ?? body.bonusDate)
        : monthRaw
          ? normalizeMonthStart(monthRaw)
          : undefined;
    ...
    const requestedMode =
      amountModeRaw === undefined
        ? undefined
        : amountModeRaw
          ? String(amountModeRaw)
          : null;
    ...
    const wantsPercent =
      ...
      (requestedMode !== "fixed" && amountPercentStr !== null);
    const wantsFixed =
      requestedMode === "fixed" ||
      (requestedMode !== "percent" && amountFixedStr !== null);
    ...
    if (wantsFixed) {
      ...
      if (!resolvedEmployeeId) {
        const [current] = await sql(
          `SELECT employee_id FROM hr_employee_bonuses WHERE id = $1`,
          [bonusId],
        );
        resolvedEmployeeId = current?.employee_id
    ...
    return Response.json(
      { error: "Failed to update bonus", details: error.message },
      { status: 500 },
    );
  }
}
...
export async function DELETE(request, { params: { id } }) {
  ...
  try {
    ...
    return Response.json({ error: "Failed to delete bonus" }, { status: 500 });
  }
}
