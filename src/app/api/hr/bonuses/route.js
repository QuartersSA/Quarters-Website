...
async function notifyBonusWhatsApp({ createdRows, actorUser }) {
  try {
    ...
    const dateValue = rows[0]?.bonus_date
      ? String(rows[0].bonus_date).slice(0, 10)
      : null;

    const categoryValue = rows[0]?.bonus_category || null;
    const reasonValue = rows[0]?.reason || null;

    const createdByUsername = actorUser?.username
      ? String(actorUser.username)
      : null;
    ...
    const affectedIds = Array.from(
      new Set(rows.map((r) => safeInt(r.employee_id)).filter(Boolean)),
    );
    ...
    hrRecipients.forEach((r) =>
      recipientsMap.set(Number(r.id), { ...r, kind: "hr" }),
    );
    affectedRecipients.forEach((r) => {
      const id = Number(r.id);
      if (!recipientsMap.has(id)) {
        recipientsMap.set(id, { ...r, kind: "employee" });
      }
    });

    const recipients = Array.from(recipientsMap.values());
    if (recipients.length === 0) {
      return { ok: true, skipped: true, reason: "no_phones" };
    }
    ...
    const headerLines = [
      "بونص جديد",
      rows.length > 1 ? `عدد الموظفين: ${rows.length}` : null,
      dateValue ? `التاريخ: ${dateValue}` : null,
      categoryValue ? `النوع: ${categoryValue}` : null,
    ...
    const hrText = [
      ...
      ...listLines,
      moreLine,
      reasonValue ? `السبب: ${reasonValue}` : null,
    ]
      .filter(Boolean)
      .join("\n")
      .trim();
    ...
    const results = await Promise.all(
      recipients.map(async (r) => {
        const to = r.phone;
        if (!to) return { employeeId: r.id, ok: true, skipped: true };
        ...
        const res = await sendWhatsAppViaWasender({ to, text });
        if (!res.ok) {
        ...
        return { employeeId: r.id, ok: res.ok };
      }),
    );
...
export async function GET(request) {
  ...
  try {
    ...
    where +=
      " ORDER BY b.bonus_date DESC, b.created_at DESC, b.id DESC LIMIT 500";

    const rows = await buildBonusRowSelectSql(where, params);
    return Response.json(rows);
  } catch (error) {
    console.error("HR: Error fetching bonuses:", error);
    return Response.json({ error: "Failed to fetch bonuses" }, { status: 500 });
  }
}
...
export async function POST(request) {
  ...
  try {
    ...
    const bonusDate = normalizeIsoDate(
      body.bonus_date ??
        body.bonusDate ??
        (monthRange ? monthRange.monthStart : null),
    );
    ...
    const amountPercentStr = safeNumericString(
      body.amount_percent ?? body.amountPercent,
    );
    ...
    const usingPercent =
      ...
      (amountFixedStr === null && amountPercentStr !== null);
    ...
    if (usingPercent) {
      ...
      computedAmounts = new Map(
        (rows || []).map((r) => [Number(r.id), r.amount ?? 0]),
      );
    }

    const insertResults = await sql.transaction((txn) =>
      uniqueEmployeeIds.map((empId) => {
        const amountValue = usingPercent
          ? (computedAmounts.get(Number(empId)) ?? 0)
          : amountFixedStr;
    ...
    const rows = newIds.length
      ? await buildBonusRowSelectSql(
          `WHERE b.id = ANY($1::int[]) ORDER BY b.bonus_date DESC, b.created_at DESC, b.id DESC`,
          [newIds],
        )
      : [];

    try {
      await notifyBonusWhatsApp({ createdRows: rows, actorUser: auth.user });
    } catch (e) {
    ...
    return Response.json(
      { error: "Failed to create bonus", details: error.message },
      { status: 500 },
    );
  }
}
