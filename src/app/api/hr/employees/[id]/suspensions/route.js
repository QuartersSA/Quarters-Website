  ...
  try {
    ...
    const rows = includeInactive
      ...
      : await sql`
        ...
        `;
    return Response.json({ suspensions: rows });
  } catch (error) {
    console.error("hr suspensions GET", error);
    return Response.json(
      { error: "فشل تحميل الإيقافات", details: error.message },
      { status: 500 },
    );
  }
}
...
export async function POST(request, { params }) {
  ...
  try {
    ...
    return Response.json({ ok: true, suspension: created });
  } catch (error) {
    console.error("hr suspensions POST", error);
    return Response.json(
      { error: "فشل إضافة الإيقاف", details: error.message },
      { status: 500 },
    );
  }
}
