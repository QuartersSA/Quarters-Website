...
export async function PUT(request, { params }) {
  ...
  try {
    ...
    if (
      health_card_expiry_date !== undefined &&
    ...
    if (updates.length > 0) {
      ...
      const [exists] =
        await sql`SELECT id FROM employees WHERE id = ${employeeId}`;
      if (!exists) {
        return Response.json({ error: "Employee not found" }, { status: 404 });
      }
    }
    ...
    const branchesChanged =
      JSON.stringify(beforeBranchIds) !== JSON.stringify(afterBranchIds);
