...
export async function POST(request) {
  ...
  try {
    ...
    if (!phone) {
      return Response.json({ error: "أدخل رقم المستلم" }, { status: 400 });
    }
    ...
    const text = [
      "✅ رسالة تجريبية — نظام كوارترز",
      "ربط الواتساب يعمل بنجاح.",
      `الوقت: ${now} (الرياض)`,
      auth.user?.name ? `أرسلها: ${auth.user.name}` : null,
    ]
      .filter(Boolean)
      .join("\n");

    const result = await sendWhatsAppViaWasender({ to: phone, text });
    if (!result.ok) {
