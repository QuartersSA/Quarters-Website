...
export function verifySessionToken(token) {
  try {
    ...
    if (parts.length !== 2) {
      return { ok: false, error: "bad_format", payload: null };
    }
    ...
    return { ok: false, error: "verify_failed", payload: null };
  }
}
...
export function requireAuth(request, { role, permission, anyOf } = {}) {
  ...
  if (Array.isArray(anyOf) && anyOf.length > 0) {
    ...
    if (!ok) {
      return { ok: false, status: 403, error: "Forbidden", user: null };
    }
