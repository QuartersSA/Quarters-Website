...
export async function ensureCoffeeSchema() {
  if (ensured) return;
...
export async function getRoastingAccountId() {
  const [row] = await sql`
...
export async function loadBeanInfo(accountIds = []) {
  ...
  for (const row of rows) {
    const isBean =
      !!row.item_id && row.item_active !== false && row.is_roasted_coffee === true;
...
export async function resolveKgPerBaseUnit(itemId) {
  const [row] = await sql`
...
export async function applyCoffeeToItems(items, payload, { existingLines = [], invoiceKind = "purchase" } = {}) {
  ...
  return items.map((item, index) => {
    ...
    const base = { ...item, lineDiscount, lineNet };
    ...
    const enabled = enabledRaw === undefined || enabledRaw === null
      ? !!existing?.roast_enabled
      : enabledRaw === true;
    if (!enabled) return { ...base, coffee: null };
    ...
    if (!bean?.isBean) {
      ...
      throw new CoffeeError(
        400,
        `البند «${item.description || index + 1}»: فعّل التحميص على حساب نوع بن (صنف تحت «بن قهوة محمصة») — لا على التصنيف أو حساب يدوي`,
      );
    }
    if (item.includesTax) {
    ...
    if (!freeSample && c.rawCostPerKg !== null && (c.rawCostPerKg < RAW_PRICE_MIN || c.rawCostPerKg > RAW_PRICE_MAX) && item.confirm_unusual_price !== true) {
      throw new CoffeeError(
        400,
        `البند «${item.description || bean.itemName}»: سعر الكيلو الخام غير منطقي (${c.rawCostPerKg} ر.س/كغ) — تحقق من وحدة الكمية (خيشة/كغ) أو أكّد السعر`,
        { code: "unusual_price", raw_cost_per_kg: c.rawCostPerKg },
      );
    }
    if (arrivalComplete && receivedKg > c.rawKg * (roastPerKg === 0 ? 1.02 : 1) + 0.0005) {
      throw new CoffeeError(409, `البند «${item.description || bean.itemName}»: الكمية الواصلة المسجلة (${receivedKg} كغ) أكبر من الكيلو الخام الجديد — صحّح الوصول أولًا`, { code: "reconfirm_received" });
    }
...
export const LINE_SELECT_COLUMNS = `
...
export async function loadInvoiceLines(invoiceId) {
  return sql(
    `SELECT ${LINE_SELECT_COLUMNS} FROM accounting_purchase_invoice_items
...
export function insertLineStatement(id, invoiceId, line, extra = {}) {
...
export function updateLineStatement(line) {
  const c = coffeeCols(line);
  return sql`
...
export async function reserveIds(table, count) {
  if (count <= 0) return [];
  const rows = await sql(
...
export async function planLineReconcile(invoiceId, items) {
  ...
  for (const gone of deletes) {
    if (gone.arrival_complete || Number(gone.deposited_kg) > 0) {
      throw new CoffeeError(
        409,
        `لا يمكن حذف البند «${gone.description || gone.bean_name || gone.id}» — له وصول مسجَّل${Number(gone.deposited_kg) > 0 ? " وإيداع في المخزون" : ""}. ألغِ الوصول أولًا من نافذة تسجيل الوصول`,
        { code: "deposited_line" },
      );
    }
  }
  const newIds = await reserveIds("accounting_purchase_invoice_items", inserts.length);
  inserts.forEach((line, i) => { line.id = newIds[i]; });
  const statements = [
    ...deletes.map((l) => sql`DELETE FROM accounting_purchase_invoice_items WHERE id = ${l.id}`),
    ...updates.map((l) => updateLineStatement(l)),
    ...inserts.map((l) => insertLineStatement(l.id, invoiceId, l)),
  ];
  return { statements, lines: [...updates, ...inserts], existing, deletes };
}

// ── فاتورة التحميص ────────────────────────────────────────────────

export async function loadRoastChild(beanInvoiceId) {
...
export async function syncRoastInvoice(bean, beanLines, actor, { allowCreate = true } = {}) {
  const desired = desiredRoastLines(beanLines);
  const child = await loadRoastChild(bean.id);
  const roastingAccountId = await getRoastingAccountId();

  if (!child) {
    if (!desired.length || !allowCreate) return { action: "none" };
    ...
    const statements = [
      ...
      `,
      ...desired.map((l, i) =>
        insertLineStatement(lineIds[i], invoiceId, {
          position: i, description: l.description, accountId: roastingAccountId,
          quantity: l.quantity, unitPrice: l.unitPrice, amount: l.amount,
          taxRate: l.taxRate, includesTax: false, subtotal: l.subtotal, tax: l.tax,
          total: l.total, lineDiscount: 0, lineNet: l.subtotal, coffee: null,
        }, { roastForItemId: l.roastForItemId }),
      ),
    ];
    await sql.transaction(statements);
    await logPurchaseAudit({
      entityType: "invoice", entityId: invoiceId, action: "roast_generated",
      summary: `توليد فاتورة التحميص ROAST-${bean.id} على ${roaster.name} بمبلغ ${total.toFixed(2)} SAR من فاتورة البن ${bean.invoice_number}`,
      actor,
    });
    return { action: "created", roastInvoiceId: invoiceId, total };
  }

  // طفل قائم — فاتورة درر مؤكَّدة = الحقيقة، لا تُلمس من جهة البن.
  if (child.roast_confirmed) return { action: "confirmed_skip", roastInvoiceId: child.id };
  ...
  if (!linesChanged && !roasterChanged && !dateChanged) return { action: "unchanged", roastInvoiceId: child.id };

  if (paid > 0 && paid >= oldTotal - 0.004 && (linesChanged || roasterChanged)) {
    throw new CoffeeError(409, `فاتورة التحميص ${child.invoice_number} مسددة بالكامل — عدّل التحميص من فاتورة التحميص نفسها`, { code: "roast_paid" });
  }
  if (paid > 0 && total < paid - 0.004) {
    throw new CoffeeError(409, `فاتورة التحميص ${child.invoice_number} عليها دفعات ${paid.toFixed(2)} أكبر من الإجمالي الجديد ${total.toFixed(2)} — ألغِ الدفعة أولًا`, { code: "roast_paid_over" });
  }
  if (paid > 0 && roasterChanged) {
    throw new CoffeeError(409, `فاتورة التحميص ${child.invoice_number} عليها دفعات — لا يمكن تغيير المحمصة`, { code: "roast_paid" });
  }
  ...
  const statements = [
    sql`DELETE FROM accounting_purchase_invoice_items WHERE invoice_id = ${child.id}`,
    ...desired.map((l, i) =>
      insertLineStatement(lineIds[i], child.id, {
        position: i, description: l.description, accountId: roastingAccountId,
        quantity: l.quantity, unitPrice: l.unitPrice, amount: l.amount,
        taxRate: l.taxRate, includesTax: false, subtotal: l.subtotal, tax: l.tax,
        total: l.total, lineDiscount: 0, lineNet: l.subtotal, coffee: null,
  ...
  await sql.transaction(statements);
  await logPurchaseAudit({
    entityType: "invoice", entityId: child.id, action: "roast_synced",
  ...
  return { action: "synced", roastInvoiceId: child.id, total };
}
...
export async function assertRoastSyncAllowed(beanInvoiceId, enrichedLines, roasterContactId) {
  const child = await loadRoastChild(beanInvoiceId);
  if (!child || child.roast_confirmed) return;
  const desiredTotal = round2(
    enrichedLines.reduce((s, l) => s + (l.coffee ? l.coffee.roastTotalNet + l.coffee.roastTaxAmount : 0), 0),
  );
  const paid = Number(child.paid_amount) || 0;
  const oldTotal = Number(child.total_amount) || 0;
  if (paid <= 0) return;
  ...
  if (desiredTotal < paid - 0.004) {
    throw new CoffeeError(409, `فاتورة التحميص ${child.invoice_number} عليها دفعات ${paid.toFixed(2)} أكبر من الإجمالي الجديد ${desiredTotal.toFixed(2)} — ألغِ الدفعة أولًا`, { code: "roast_paid_over" });
  }
...
export async function resolveRoaster(roasterContactId, beanLines) {
  ...
  if (!id) {
    ...
    for (const v of info.values()) if (v.isBean && v.roasterContactId) { id = v.roasterContactId; break; }
  }
  if (!id) {
  ...
  return contact ? { id: Number(contact.id), name: contact.name } : null;
}
...
export async function reverseSyncRoastToBean(roastInvoiceId, actor) {
  const [child] = await sql`
  ...
  await sql.transaction(statements);
  await logPurchaseAudit({
    entityType: "invoice", entityId: Number(child.source_invoice_id), action: "roast_reverse_synced",
...
export async function recordArrival(invoice, lines, { deposit = null, actor = null, canFinalize = false } = {}) {
  ...
  for (const input of lines) {
    ...
    if (received === null) {
      ...
      if (Number(line.deposited_kg) > 0) throw new CoffeeError(409, `البند «${line.bean_name}» مودَع في المخزون — ألغِ الإيداع أولًا`, { code: "deposited_line" });
      statements.push(sql`
        UPDATE accounting_purchase_invoice_items
    ...
    const c = computeCoffeeLine({
      ...
      roastPerKg, roastTaxRate: Number(line.roast_tax_rate) || 0, extraCost: Number(line.extra_cost) || 0,
      receivedKg: received, arrivalComplete: complete,
    });
    if (complete && c.wastePercent !== null && c.wastePercent > WASTE_CONFIRM && input.confirm_high_waste !== true) {
      throw new CoffeeError(400, `البند «${line.bean_name}»: نسبة الهدر ${c.wastePercent}% غير اعتيادية — أكّدها مع ملاحظة`, { code: "high_waste", waste_percent: c.wastePercent });
    }
    ...
    if (deposit?.enabled && line.item_id) {
      depositPlans.push({ line, received: round3(received), arrivalDate });
    }
  }
  ...
  await logPurchaseAudit({
    entityType: "invoice", entityId: invoice.id, action: canFinalize ? "coffee_arrival" : "coffee_arrival_report",
    summary: `${invoice.invoice_number}: ${summaries.join("؛ ")}`,
    actor,
  });
  return { touchedItems: [...touchedItems] };
}
...
async function planDeposits(invoice, plans, branchId, actor) {
  ...
  const [items, snapshots] = await Promise.all([
    sql`SELECT id, name, is_active, show_in_inventory FROM items WHERE id = ANY(${itemIds})`,
    getDefaultInventoryUnitSnapshots(itemIds),
  ]);
...
export async function reverseDeposits(invoiceId, actor) {
  ...
  await sql.transaction(statements);
  await logPurchaseAudit({
    entityType: "invoice", entityId: invoiceId, action: "coffee_deposit_reversed",
...
export async function recomputeItemCost(itemId, actor = null) {
  ...
  if (!kgPerBase) {
    await logPurchaseAudit({ entityType: "item", entityId: id, action: "cost_sync_skipped", summary: `لم تُحدَّث تكلفة الصنف #${id}: بلا وحدة أساس كيلو/جرام`, actor });
    return null;
  }
...
export async function anyCoffeeAccount(accountIds = []) {
  const info = await loadBeanInfo(accountIds);
...
export async function loadRoastLinks(invoiceIds = []) {
