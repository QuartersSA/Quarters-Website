...
// نواة التحليل الذكي لفواتير المشتريات — مشتركة بين مسارين:
//   - الفاتورة الواحدة:  POST /api/accounting/purchase-invoices/analyze
//   - الرفع الجماعي:     PATCH /api/accounting/purchase-invoice-batches/[id]
// أي تحسين على الاستخراج هنا يستفيد منه المساران معاً.
//
// يتطلب ANTHROPIC_API_KEY في البيئة؛ بدونه يرجع { ok:false, status:503 }.
...
const OUTPUT_SCHEMA = {
  ...
  properties: {
    ...
    supplier_vat_number: {
      type: ["string", "null"],
      description:
        "SELLER's VAT registration number as printed, digits only (KSA: 15 digits starting with 3). NOT the buyer/customer VAT. Null when absent.",
    },
    contact_id: {
      type: ["integer", "null"],
    ...
    discount: {
      type: ["number", "null"],
      description:
        "Invoice-level discount amount applied BEFORE tax, as printed. Null/0 when none.",
    },
...
// تشغيل التحليل الكامل. المدخلات نفسها التي كان يقبلها مسار الفاتورة
// الواحدة؛ الإرجاع: { ok:true, analysis } أو { ok:false, status, error }.
export async function runInvoiceAnalysis({
  fileBase64 = "",
  mediaType = "",
  text = "",
}) {
  ...
  const hasFile =
    fileBase64.length > 0 && FILE_MEDIA_TYPES.has(mediaType);
  ...
  if (!hasFile && trimmedText.trim().length < 10) {
    return { ok: false, status: 400, error: "نص الفاتورة فارغ" };
  }
            ...
            : []),
          {
            ...
            text: [
              ...
              ...(trimmedText.trim().length >= 10
                ? [
                    "",
                    "## نص الفاتورة المستخرج آلياً (مساعد ثانوي)",
                    trimmedText,
                  ]
                : []),
            ].join("\n"),
  ...
  if (items.length > 0 && Number.isFinite(grandTotal) && grandTotal > 0) {
    ...
    if (
      ...
      netSum > 0 &&
      grossSum > grandTotal &&
      !close(grossSum, grandTotal)
    ) {
      // total = grossSum × (1 − D/netSum)  ⟹  D:
      const derived =
        Math.round(netSum * (1 - grandTotal / grossSum) * 100) / 100;
