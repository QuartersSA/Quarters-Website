...
export async function GET(request) {
  try {
    ...
    return Response.json({ error: "فشل تحميل المهام" }, { status: 500 });
  }
}
...
export async function POST(request) {
  try {
    ...
    if (!title) {
      return Response.json({ error: "عنوان المهمة مطلوب" }, { status: 400 });
    }

    // NEW: require due date + space when creating a task
    if (!dueDate) {
      return Response.json({ error: "تاريخ الاستحقاق مطلوب" }, { status: 400 });
    }

    if (!spaceId) {
      return Response.json({ error: "المساحة مطلوبة" }, { status: 400 });
    }

    if (spaceId) {
      const [space] =
        await sql`SELECT id FROM workspace_spaces WHERE id = ${spaceId}`;
      if (!space) {
        return Response.json({ error: "المساحة غير موجودة" }, { status: 400 });
      }
    }

    const requestedAssignees = normalizeAssigneeIds(
    ...
    if (requestedAssignees.length > 0) {
      ...
      const rows = await sql(
        `SELECT id FROM employees WHERE id IN (${placeholders.join(", ")})`,
        vals,
      );
    ...
    return Response.json(
      details
        ? { error: "فشل إنشاء المهمة", details }
