...
export async function GET(request, { params }) {
  try {
    ...
    if (!parentId) {
      return Response.json({ error: "parentId غير صحيح" }, { status: 400 });
    }
...
export async function POST(request, { params }) {
  try {
    ...
    const parentRows =
      await sql`SELECT id, space_id, due_date, title FROM workspace_tasks WHERE id = ${parentId}`;
    ...
    const assigneeIds = [];
    const rawIds = Array.isArray(body.assigneeEmployeeIds)
    ...
    if (task?.id && assigneeIds.length > 0) {
      ...
      await sql(
        `INSERT INTO workspace_task_assignees (task_id, employee_id) VALUES ${tuples.join(", ")} ON CONFLICT DO NOTHING`,
        insertValues,
      );
    }
    ...
    if (task?.id) {
      await sql(
        ...
        [
          ...
          employeeId,
          "تم إنشاء مهمة فرعية",
          JSON.stringify({ parent_task_id: parentId, title: task.title }),
        ],
      );
    }
    ...
    if (task?.id) {
      ...
      const lines = [
        ...
        formatTaskLine("المهمة الرئيسية", parent.title || `#${parentId}`),
        ...
        formatTaskLine("أنشأها", actorName),
...
export async function PATCH(request, { params }) {
  try {
    ...
    // Check parent exists
    const parentRows =
      await sql`SELECT id FROM workspace_tasks WHERE id = ${parentId}`;
    ...
    const childRows =
      await sql`SELECT id, parent_task_id FROM workspace_tasks WHERE id = ${childTaskId}`;
    if (!childRows.length) {
      return Response.json(
    ...
    const circularCheck =
      await sql`SELECT id FROM workspace_tasks WHERE parent_task_id = ${childTaskId} LIMIT 1`;
    ...
    const parentParentCheck =
      await sql`SELECT parent_task_id FROM workspace_tasks WHERE id = ${parentId}`;
    ...
    return Response.json({ error: "فشل ربط المهمة الفرعية" }, { status: 500 });
  }
}
