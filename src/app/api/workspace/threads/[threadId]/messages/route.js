...
export async function GET(request, { params }) {
  try {
    ...
    if (!threadId) {
      return Response.json({ error: "threadId غير صحيح" }, { status: 400 });
    }
...
export async function POST(request, { params }) {
  try {
    ...
    if (!bodyText) {
      return Response.json({ error: "نص الرسالة مطلوب" }, { status: 400 });
    }
    ...
    return Response.json({ error: "فشل إرسال الرسالة" }, { status: 500 });
  }
}
