...
export async function POST(request) {
  try {
    ...
    if (existing.length > 0) {
      return Response.json({ threadId: existing[0].id, reused: true });
    }

    // Create thread and add members
    const [created] = await sql(
      `INSERT INTO workspace_threads (kind, title, created_by_employee_id) VALUES ('dm', $1, $2) RETURNING id`,
      [title, employeeId],
    );

    await sql(
      `INSERT INTO workspace_thread_members (thread_id, employee_id) VALUES ($1, $2), ($1, $3)`,
      [created.id, employeeId, otherEmployeeId],
    );

    return Response.json({ threadId: created.id, reused: false });
  } catch (error) {
    console.error("workspace threads POST error:", error);
    return Response.json({ error: "فشل إنشاء محادثة" }, { status: 500 });
  }
}
