  ...
  try {
    ...
    if (body.expense_type_id !== undefined) {
      ...
      if (!Number.isFinite(v) || v <= 0) {
        return Response.json({ error: "التصنيف غير صالح" }, { status: 400 });
      }
...
export async function DELETE(request, { params }) {
  ...
  try {
    ...
    if (force) {
      const stmts = [
        sql`DELETE FROM accounting_expenses WHERE variable_template_id = ${id}`,
        sql`DELETE FROM accounting_variable_templates WHERE id = ${id} RETURNING id`,
      ];
