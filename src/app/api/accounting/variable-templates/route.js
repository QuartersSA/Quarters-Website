...
export async function GET(request) {
  ...
  try {
    ...
    const rows = includeInactive
      ? await sql`
          SELECT v.*, t.name AS expense_type_name
    ...
    return Response.json(
      { error: "فشل تحميل قوالب المصروفات المتغيرة", details: error.message },
      { status: 500 },
    );
  }
}
...
export async function POST(request) {
  ...
  try {
    ...
    if (!name) {
      return Response.json({ error: "اسم البند مطلوب" }, { status: 400 });
    }
    if (!Number.isFinite(typeId) || typeId <= 0) {
      return Response.json({ error: "التصنيف مطلوب" }, { status: 400 });
    }
