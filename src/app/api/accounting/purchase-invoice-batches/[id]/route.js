...
import {
  ...
  readUploadBase64,
} from "@/app/api/utils/invoiceBatches";
import {
  runInvoiceAnalysis,
  FILE_MEDIA_TYPES,
} from "@/app/api/utils/invoiceAnalysis";
import { computeDraftTotals, round2 } from "@/utils/invoiceDraftMath";
...
function buildDraft(analysis) {
  const items = (Array.isArray(analysis?.items) ? analysis.items : [])
    .map((item) => ({
      ...
      tax_rate: Number.isFinite(Number(item.tax_rate))
  ...
  return {
    ...
    supplier_name: analysis?.supplier_name
      ? String(analysis.supplier_name)
      : "",
    supplier_vat_number: analysis?.supplier_vat_number
...
function sanitizeDraft(input) {
  ...
  return {
    ...
    contact_matched_by: input.contact_matched_by
      ? text(input.contact_matched_by, 20)
      : null,
...
// أم «يحتاج انتباهاً».
function computeFlags(draft, { duplicateInvoice, duplicateItem, note }) {
  const flags = [];
...
async function findDuplicates({ batchId, itemId, draft }) {
...
async function loadOwnedBatch(batchId, user) {
  ...
  `;
  if (!batch) return { error: "الدفعة غير موجودة", status: 404 };
  const isAdmin = user?.role === "Admin";
  if (
    !isAdmin &&
    Number(batch.created_by_employee_id) !== Number(user?.id || 0)
  ) {
    return { error: "ليست لديك صلاحية على هذه الدفعة", status: 403 };
  }
  return { batch };
}
...
export async function GET(request, { params: { id } }) {
  ...
  try {
    ...
    const items = await sql`
      ...
      ORDER BY id ASC
    `;
    return Response.json({ batch: owned.batch, items });
  } catch (error) {
    console.error("invoice batch get error", error);
    return Response.json(
      { error: "فشل تحميل الدفعة", details: error.message },
      { status: 500 },
    );
  }
}
...
export async function PATCH(request, { params: { id } }) {
  ...
  try {
    ...
    `;
    if (!item) {
      return Response.json({ error: "البند غير موجود" }, { status: 404 });
    }
    ...
    if (action === "analyze") {
      ...
      const fail = async (message, status = "failed") => {
        await sql`
          ...
          WHERE id = ${itemId}
        `;
        return Response.json({ ok: true, item_status: status, error: message });
      };
      ...
      if (file.tooLarge) {
        return fail(
          "حجم الملف يتجاوز حد التحليل الذكي (3MB) — أدخل البيانات يدوياً",
          "needs_attention",
        );
      }
      let result;
      ...
      const { duplicateInvoice, duplicateItem } = await findDuplicates({
        batchId,
        itemId,
        draft,
      });
      ...
      const status = flags.some((flag) => ATTENTION_FLAGS.has(flag))
        ? "needs_attention"
        : "ready";
      ...
      return Response.json({ ok: true, item_status: status });
    }
    ...
    if (action === "save_draft") {
      ...
      if (!draft) {
        return Response.json({ error: "مسودة غير صالحة" }, { status: 400 });
      }
      const { duplicateInvoice, duplicateItem } = await findDuplicates({
        batchId,
        itemId,
      ...
      return Response.json({ ok: true, item_status: status, flags });
    }
    ...
    if (action === "approve") {
      ...
      if (!draft) {
        return Response.json({ error: "لا مسودة للاعتماد" }, { status: 400 });
      }
      const totals = computeDraftTotals(draft);
        ...
        return Response.json(
          {
            error: duplicateInvoice
              ? `يبدو أنها مكررة — فاتورة ${duplicateInvoice.invoice_number} بتاريخ ${duplicateInvoice.invoice_date} بمبلغ ${duplicateInvoice.total_amount} موجودة مسبقاً`
              : `رقم الفاتورة مكرر داخل الدفعة (ملف ${duplicateItem.file_name || duplicateItem.id})`,
            duplicate: true,
          },
          { status: 409 },
        );
      ...
      return Response.json({ ok: true, item_status: "approved" });
    }
    ...
    if (action === "submit") {
      ...
      const items = (Array.isArray(draft?.items) ? draft.items : [])
        .map((line) => ({
          ...
          tax_rate: Number.isFinite(Number(line.tax_rate))
      ...
      try {
        ...
        created = { ok: false, status: 500, error: error.message };
      }
      if (!created.ok) {
    ...
    if (action === "retry") {
      ...
      `;
      return Response.json({ ok: true, item_status: "uploaded" });
    }
    ...
    return Response.json(
      { error: "فشل تنفيذ الإجراء", details: error.message },
      { status: 500 },
    );
  }
}
