...
export async function GET(request) {
  ...
  try {
    ...
    return Response.json(
      { error: "فشل تحميل دفعات الرفع", details: error.message },
      { status: 500 },
    );
  }
}
...
export async function POST(request) {
  ...
  try {
    ...
    if (!items.length) {
      return Response.json({ error: "لا توجد ملفات في الدفعة" }, { status: 400 });
    }
    ...
    const [batch] = await sql`
      ...
      RETURNING id
    `;
    for (const { item, session } of verified) {
      await sql`
    ...
    return Response.json({ ok: true, batch_id: batch.id }, { status: 201 });
  } catch (error) {
    console.error("invoice batch create error", error);
    return Response.json(
      { error: "فشل إنشاء الدفعة", details: error.message },
      { status: 500 },
    );
  }
}
