...
export async function GET(request) {
  ...
  try {
    ...
    if (monthRaw && /^\d{4}-\d{2}$/.test(monthRaw)) {
      ...
      conditions.push(
        `l.start_month <= $${idx} AND (l.start_month + (l.installments_count || ' months')::interval) > $${idx}`,
      );
    ...
    return Response.json({ loans: rows });
  } catch (error) {
    console.error("employee-loans GET error", error);
    return Response.json(
      { error: "فشل تحميل السلف", details: error.message },
      { status: 500 },
    );
  }
}
...
export async function POST(request) {
  ...
  try {
    ...
    return Response.json(
      { error: "فشل إضافة القرض", details: error.message },
      { status: 500 },
    );
  }
}
