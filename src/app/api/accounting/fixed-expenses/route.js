...
export async function GET(request) {
  ...
  try {
    ...
    return Response.json({ fixed_expenses: rows });
  } catch (error) {
    console.error("fixed-expenses GET error", error);
    return Response.json(
      { error: "فشل تحميل المصروفات الثابتة", details: error.message },
      { status: 500 },
    );
  }
}
...
export async function POST(request) {
  ...
  try {
    ...
    const expenseName = body.expense_name
      ...
      : "";
    const defaultAmount =
      body.default_amount !== undefined && body.default_amount !== null
        ? Number(body.default_amount)
        : null;
    const startMonthRaw = body.start_month
      ? String(body.start_month).trim()
      : "";
    const frequency =
      body.frequency && VALID_FREQ.has(body.frequency)
        ? body.frequency
        : "monthly";
    ...
    return Response.json({ ok: true, fixed_expense: created });
  } catch (error) {
    console.error("fixed-expenses POST error", error);
    return Response.json(
      { error: "فشل إضافة المصروف الثابت", details: error.message },
      { status: 500 },
    );
  }
}
