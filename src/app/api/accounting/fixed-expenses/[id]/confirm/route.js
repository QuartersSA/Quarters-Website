...
export async function POST(request, { params }) {
  ...
  try {
    ...
    const confirmedAmount =
      body.confirmed_amount !== undefined &&
      body.confirmed_amount !== null &&
      body.confirmed_amount !== ""
        ? Number(body.confirmed_amount)
        : Number(tpl.default_amount || 0);
    ...
    const confirmedNote = body.confirmed_note
      ? String(body.confirmed_note).trim() || null
      : null;
    ...
    return Response.json({ ok: true, expense: resultRow });
  } catch (error) {
    console.error("fixed-expense confirm error", error);
    return Response.json(
      { error: "فشل تأكيد دفع المصروف الثابت", details: error.message },
      { status: 500 },
    );
  }
}
