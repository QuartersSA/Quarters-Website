...
export async function PATCH(request, { params }) {
  ...
  try {
    ...
    const query = `UPDATE accounting_fixed_expenses SET ${setClauses.join(
      ", ",
    ...
    return Response.json({ ok: true, fixed_expense: updated });
  } catch (error) {
    console.error("fixed-expense PATCH error", error);
    return Response.json(
      { error: "فشل تحديث المصروف الثابت", details: error.message },
      { status: 500 },
    );
  }
}
...
export async function DELETE(request, { params }) {
  ...
  try {
    ...
    if (hard) {
      ...
      if (refs > 0) {
        return Response.json(
          {
            error:
              "لا يمكن الحذف نهائياً لأن هناك مصروفات مؤكدة مرتبطة. استخدم إلغاء التنشيط بدلاً من ذلك.",
          },
          { status: 409 },
        );
      }
    ...
    return Response.json(
      { error: "فشل حذف المصروف الثابت", details: error.message },
      { status: 500 },
    );
  }
}
