...
export async function POST(request, { params }) {
  ...
  try {
    ...
    if (existing) {
      ...
      return Response.json({ ok: true, paid: false, expense: null });
    }
    ...
    const cycleMonths =
      template.frequency === "quarterly"
        ? 3
        : template.frequency === "semi_annual"
          ? 6
    ...
    const overrideAmount =
      body.amount !== undefined && body.amount !== null && body.amount !== ""
        ...
        : null;
    const derivedAmount =
      ((Number(template.default_amount) || 0) / cycleMonths);
    const amount =
      overrideAmount !== null &&
      Number.isFinite(overrideAmount) &&
      overrideAmount >= 0
        ? overrideAmount
        : Math.round(derivedAmount * 100) / 100;
    ...
    return Response.json({ ok: true, paid: true, expense });
  } catch (error) {
    console.error("fixed-expenses toggle-paid error", error);
    return Response.json(
      { error: "فشل تحديث الحالة", details: error.message },
      { status: 500 },
    );
  }
}
