...
import { ensurePurchaseAuditSchema } from "@/app/api/utils/purchaseAudit";
...
export async function GET(request) {
  ...
  try {
    ...
    const limit =
      Number.isInteger(limitRaw) && limitRaw > 0 && limitRaw <= 1000
        ? limitRaw
        : 400;
    ...
    if (q) {
      conditions.push(
        `(LOWER(COALESCE(summary,'')) LIKE $${idx} OR LOWER(COALESCE(actor_name,'')) LIKE $${idx} OR LOWER(action) LIKE $${idx})`,
      );
      values.push(`%${q}%`);
      idx += 1;
    }
    ...
    return Response.json(
      { error: "فشل تحميل سجل النشاط", details: error.message },
      { status: 500 },
    );
  }
}
