...
const OUTPUT_SCHEMA = {
  ...
  properties: {
    ...
    tax: { type: ["number", "null"], description: "Total VAT amount" },
    items: {
      type: "array",
      items: {
        type: "object",
        ...
        properties: {
          ...
          tax_rate: { type: "number", description: "Percent, e.g. 15" },
          amount_includes_tax: {
            type: "boolean",
    ...
    operator_note: {
      type: ["string", "null"],
      description:
        "Short Arabic note for the operator when something needs review (garbled rows, uncertain totals). Null when clean.",
    },
  },
};
...
10. الخصم: إذا تضمنت الفاتورة خصماً إجمالياً، وزّعه على البنود بتخفيض أسعار وحداتها تناسبياً بحيث يساوي مجموع البنود الإجمالي النهائي بعد الخصم، واذكر قيمة الخصم في operator_note.
11. مجموع البنود يجب أن يطابق الإجمالي. البند المجمّع الواحد بوصف "إجمالي الفاتورة" حل أخير فقط عندما يستحيل تمييز أسطر المنتجات إطلاقاً — واذكر السبب في operator_note.
...
export async function POST(request) {
  ...
  try {
    ...
    if (!hasFile && text.trim().length < 10) {
      return Response.json({ error: "نص الفاتورة فارغ" }, { status: 400 });
    }
          ...
          content: [
              ...
              : []),
            {
              ...
              text: [
                ...
                "",
                "## شجرة حسابات المصروفات (القابلة للترحيل)",
                JSON.stringify(accounts),
                ...(text.trim().length >= 10
                  ? ["", "## نص الفاتورة المستخرج آلياً (مساعد ثانوي)", text]
                  : []),
              ].join("\n"),
            },
          ],
    ...
    if (!textBlock?.text) {
      return Response.json({ error: "رد فارغ من المحلل" }, { status: 502 });
    }
    ...
    if (!analysis.contact_id && analysis.supplier_name) {
      const normalizeName = (value) =>
        String(value || "")
          ...
          .replace(/ة/g, "ه")
          .replace(/ى/g, "ي")
          .replace(/(^|\s)(شركه|مؤسسه|محل|مصنع|متجر|مخبز|مقهى)(\s|$)/g, " ")
          .replace(/(^|\s)ال(?=\S)/g, "$1")
          .replace(/[^\p{L}\p{N} ]/gu, " ")
          .replace(/\s+/g, " ")
          .trim();
    ...
    if (items.length > 0 && Number.isFinite(grandTotal) && grandTotal > 0) {
      ...
      const close = (a, b) =>
        Math.abs(a - b) <= Math.max(0.02 * items.length, 0.05);
      if (
        items.some((item) => !item.amount_includes_tax) &&
        close(sumRaw, grandTotal) &&
        !close(sumPlusTax, grandTotal)
      ) {
      ...
      // Undistributed invoice-level discount (or surcharge): the item
      // sum still misses the grand total under BOTH readings of the
      // flag. Scale unit prices proportionally — what gets posted must
      // equal what the invoice actually charges.
      const effectiveSum = items.reduce((acc, item) => {
        const gross =
          Number(item.quantity) *
          Number(item.unit_price) *
          (item.amount_includes_tax
            ? 1
            : 1 + (Number(item.tax_rate) || 0) / 100);
        return acc + gross;
      }, 0);
      if (
        effectiveSum > 0 &&
        !close(effectiveSum, grandTotal) &&
        grandTotal / effectiveSum > 0.5 &&
        grandTotal / effectiveSum < 1.5
      ) {
        const factor = grandTotal / effectiveSum;
        for (const item of items) {
          item.unit_price =
            Math.round(Number(item.unit_price) * factor * 10000) / 10000;
        }
        const diff = Math.round((effectiveSum - grandTotal) * 100) / 100;
        const note =
          diff > 0
            ? `تم توزيع خصم ${diff} على أسعار الوحدات ليطابق مجموع البنود إجمالي الفاتورة.`
            : `تمت تسوية فرق ${Math.abs(diff)} على أسعار الوحدات ليطابق مجموع البنود إجمالي الفاتورة.`;
