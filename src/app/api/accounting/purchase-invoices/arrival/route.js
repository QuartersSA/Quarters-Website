...
export async function POST(request) {
  ...
  try {
    ...
    if (invoice.is_active === false) {
      return Response.json({ error: "الفاتورة موقوفة", code: "inactive_invoice" }, { status: 409 });
    }
    if (invoice.invoice_kind === "roast") {
      return Response.json({ error: "فاتورة التحميص لا تحمل وصولًا — سجّله على فاتورة البن" }, { status: 400 });
    }
    ...
    if (body.reverse_deposit === true) {
      if (!finalize) return Response.json({ error: "Forbidden" }, { status: 403 });
      ...
      return Response.json({ ok: true, reversed: count });
    }

    const lines = Array.isArray(body.lines) ? body.lines : [];
    if (!lines.length) return Response.json({ error: "لا بنود لتسجيل وصولها" }, { status: 400 });
    ...
    if (finalize && body.deposit && body.deposit.enabled) {
      ...
      const inventoryOnly =
        !auth.user?.can_manage_accounting && !auth.user?.can_manage_purchases;
      if (inventoryOnly && invoice.branch_id && Number(invoice.branch_id) !== branchId) {
        return Response.json({ error: "يمكن الإيداع في فرع الفاتورة فقط" }, { status: 403 });
      }
      deposit = { enabled: true, branch_id: branchId };
    }
    ...
    if (error instanceof CoffeeError) {
      return Response.json({ error: error.message, ...(error.extra || {}) }, { status: error.status || 400 });
    }
    console.error("coffee arrival POST error", error);
    return Response.json({ error: "فشل تسجيل الوصول", details: error.message }, { status: 500 });
  }
}
