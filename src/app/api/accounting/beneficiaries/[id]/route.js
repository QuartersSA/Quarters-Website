...
export async function PUT(request, { params }) {
  ...
  try {
    ...
    const beneficiaryType = ["bank", "government"].includes(
    ...
    if (body.swift !== undefined) {
      sets.push(`swift = $${idx}`);
      values.push(
        body.swift ? String(body.swift).trim().toUpperCase() : null,
      );
      idx += 1;
    }
    ...
    return Response.json({ ok: true, beneficiary: updated });
  } catch (error) {
