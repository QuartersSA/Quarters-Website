...
async function syncPayrollToFixedExpense({
  ...
  return { categoryId: category.id, templateId: template.id, totalAmount };
}
  ...
  try {
    ...
    try {
      if (newIsClosed) {
        ...
        // recorded.
        const [{ total_paid, total_net }] = await sql(
          `SELECT
             COALESCE(SUM(
        ...
        const totalAmount =
          Number(total_paid) > 0 ? Number(total_paid) : Number(total_net) || 0;
