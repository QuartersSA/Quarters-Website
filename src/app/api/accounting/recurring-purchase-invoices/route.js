...
import { ensureRecurringSchema } from "@/app/api/utils/purchaseAutomation";
...
function parsePayload(body = {}) {
  ...
  return {
    ...
    taxRate: Number.isFinite(rateRaw)
      ? Math.min(Math.max(rateRaw, 0), 100)
      : 15,
    amountIncludesTax: body.amount_includes_tax !== false,
    dayOfMonth:
      Number.isInteger(dayRaw) && dayRaw >= 1 ? Math.min(dayRaw, 28) : 1,
...
export async function GET(request) {
  ...
  try {
    ...
    return Response.json(
      { error: "فشل تحميل الفواتير المتكررة", details: error.message },
      { status: 500 },
    );
  }
}
...
export async function POST(request) {
  ...
  try {
    ...
    return Response.json(
      { error: "فشل إضافة القالب", details: error.message },
      { status: 500 },
    );
  }
}
...
export async function PUT(request) {
  ...
  try {
    ...
    if (!Number.isInteger(id) || id <= 0) {
      return Response.json({ error: "معرف القالب غير صحيح" }, { status: 400 });
    }
    const payload = parsePayload(body);
    const validationError = validatePayload(payload);
    if (validationError) {
    ...
    return Response.json(
      { error: "فشل تعديل القالب", details: error.message },
      { status: 500 },
    );
  }
}
...
export async function DELETE(request) {
  ...
  try {
    ...
    return Response.json(
      { error: "فشل حذف القالب", details: error.message },
      { status: 500 },
    );
  }
}
