...
export async function GET(request) {
  ...
  try {
    ...
    return Response.json(
      { error: "فشل تحميل التقارير المجدولة", details: error.message },
      { status: 500 },
    );
  }
}
...
export async function POST(request) {
  ...
  try {
    ...
    if (body.action === "send-now") {
      ...
      `;
      if (!schedule) {
        return Response.json({ error: "الجدولة غير موجودة" }, { status: 404 });
      }
    ...
    return Response.json({ ok: true, schedule: created }, { status: 201 });
  } catch (error) {
    console.error("scheduled purchase reports POST error", error);
    return Response.json(
      { error: "فشل حفظ الجدولة", details: error.message },
      { status: 500 },
    );
  }
}
...
export async function PUT(request) {
  ...
  try {
    ...
    if (!Number.isInteger(id) || id <= 0) {
      return Response.json({ error: "معرف الجدولة غير صحيح" }, { status: 400 });
    }
    const payload = parsePayload(body);
    const validationError = validatePayload(payload);
    if (validationError) {
    ...
    return Response.json(
      { error: "فشل تعديل الجدولة", details: error.message },
      { status: 500 },
    );
  }
}
...
export async function DELETE(request) {
  ...
  try {
    ...
    return Response.json(
      { error: "فشل حذف الجدولة", details: error.message },
      { status: 500 },
    );
  }
}
