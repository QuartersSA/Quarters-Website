...
export async function POST(request) {
  ...
  try {
    ...
    try {
      ...
      try {
        await sql(`DELETE FROM accounting_green_bean_orders WHERE id = $1`, [
          orderRow.id,
        ]);
      } catch (cleanupError) {
    ...
    try {
      const beanIdsInOrder = [
        ...new Set(insertedItems.map((it) => Number(it.bean_id))),
      ];
        ...
        for (const item of insertedItems) {
          ...
          if (beanId && finalPrice != null) {
            ...
            if (updated && updated.length > 0) {
              console.log(
                `Auto-updated cost for ${updated.length} item(s) linked to bean ${beanId}:`,
                updated.map((u) => `${u.name} (id=${u.id})`).join(", "),
              );
            }
          }
        }
    ...
    return Response.json({ error: "فشل إنشاء طلب البن" }, { status: 500 });
  }
}
