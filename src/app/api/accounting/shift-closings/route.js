...
async function notifyAdminsWhatsApp({
  ...
  try {
    ...
    const lines = [
      ...
      `الفرع: ${branchName || "—"}`,
      `التاريخ: ${shiftDate || "—"}`,
      shiftLabel ? `الشفت: ${shiftLabel}` : null,
      `الكاش الفعلي: ${formatMoney(actualCash)}`,
      `الشبكة الفعلية: ${formatMoney(actualCard)}`,
      `كاش فودكس: ${formatMoney(foodicsCash)}`,
      `شبكة فودكس: ${formatMoney(foodicsCard)}`,
      `فرق الكاش: ${formatMoney(cashDiff)} (${diffLabel(cashDiff)})`,
      `فرق الشبكة: ${formatMoney(cardDiff)} (${diffLabel(cardDiff)})`,
      `الإجمالي: ${formatMoney(totalDiff)} (${diffLabel(totalDiff)})`,
      employeeName ? `الموظف: ${employeeName}` : null,
      note ? `ملاحظة: ${String(note).trim()}` : null,
    ].filter(Boolean);
...
export async function GET(request) {
  ...
  try {
    ...
    return Response.json({ error: "فشل تحميل تقفيلات الشفت" }, { status: 500 });
  }
}
...
export async function POST(request) {
  const auth = requireAuth(request, {
    anyOf: [
      { role: "Employee", permission: "can_close_shift" },
  ...
  try {
    ...
    return Response.json({ error: "فشل حفظ تقفيلة الشفت" }, { status: 500 });
  }
}
