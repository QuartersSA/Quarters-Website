...
import { ensureCoffeeSchema, loadBeanInfo, resolveKgPerBaseUnit } from "@/app/api/utils/coffeeInvoices";
...
async function fetchPurchaseUnits() {
  ...
  try {
    ...
    `;
    for (const row of rows) {
      if (row.unit) units.set(Number(row.id), { unit: row.unit, factor: Number(row.factor) || 1 });
    }
  } catch {
...
export async function GET(request) {
  ...
  try {
    ...
    try {
      ...
      beanInfo = await loadBeanInfo(
        rows.filter((row) => row.source_item_id).map((row) => row.id),
      );
      ...
      return {
        ...
        bean: info?.isBean
          ? {
              ...
              purchase_unit: purchaseUnits.get(Number(row.id))?.unit || null,
              purchase_unit_kg: kgFactors.get(Number(row.id))
                ? Math.round(kgFactors.get(Number(row.id)) * (purchaseUnits.get(Number(row.id))?.factor || 1) * 1000) / 1000
                : null,
