...
export async function POST(request) {
  ...
  try {
    ...
    const kind = ["quote", "tax_invoice", "payment_receipt", "other"].includes(
      body.kind,
    )
      ? body.kind
      : /عرض\s*سعر|quotation/i.test(label || "")
        ? "quote"
        : /سداد|دفع|إيصال|سند قبض|receipt|payment/i.test(label || "")
          ? "payment_receipt"
          : /فاتورة|invoice/i.test(label || "")
            ? "tax_invoice"
            : "other";
