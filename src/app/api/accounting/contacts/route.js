...
export async function GET(request) {
  ...
  try {
    ...
    return Response.json(
      { error: "فشل تحميل جهات الاتصال", details: error.message },
      { status: 500 },
    );
  }
}
  ...
  try {
    ...
    if (vatRegistered && !vatNumber) {
      ...
      return Response.json(
        {
          error:
            "رقم التسجيل الضريبي مطلوب عندما تكون جهة الاتصال مسجلة في ضريبة القيمة المضافة",
        },
        { status: 400 },
      );
    }
    ...
    const [created] = await sql`
      ...
      RETURNING *
    `;
    return Response.json({ ok: true, contact: created });
  } catch (error) {
    console.error("contacts POST error", error);
    return Response.json(
      { error: "فشل إضافة جهة الاتصال", details: error.message },
      { status: 500 },
    );
  }
}
