...
export async function GET(request) {
  ...
  try {
    ...
    return Response.json(
      { error: "فشل تحميل سجل الدفعات", details: error.message },
      { status: 500 },
    );
  }
}
...
export async function POST(request) {
  ...
  try {
    ...
    if (amount <= 0) {
      return Response.json({ error: "مبلغ الدفعة مطلوب" }, { status: 400 });
    }
    ...
    if (invoice.created_by_employee_id) {
      ...
      const lines = [
        "💳 تم تسجيل دفعة على فاتورتك",
        `الفاتورة: ${invoice.invoice_number}`,
        invoice.supplier ? `المورد: ${invoice.supplier}` : null,
        `الدفعة: ${amount.toFixed(2)} ${invoice.currency || "SAR"} بتاريخ ${paymentDate}`,
        bankName ? `البنك: ${bankName}` : null,
        `الإجمالي المدفوع: ${newPaid.toFixed(2)} من ${total.toFixed(2)}`,
    ...
    return Response.json(
      { ok: true, payment_id: payment.id, invoice: updated },
      { status: 201 },
    );
  } catch (error) {
    console.error("invoice payments POST error", error);
    return Response.json(
      { error: "فشل تسجيل الدفعة", details: error.message },
      { status: 500 },
    );
  }
}
...
export async function DELETE(request) {
  ...
  try {
    ...
    const newPaid = Math.max(
      Math.round((Number(payment.invoice_paid) - Number(payment.amount)) * 100) /
        100,
      0,
    );
    await sql`
    ...
    return Response.json(
      { error: "فشل حذف الدفعة", details: error.message },
      { status: 500 },
    );
  }
}
