...
export async function PATCH(request) {
  ...
  try {
    ...
    const { ids, show_in_inventory } = body;
    ...
    console.log(
      `Batch updated show_in_inventory=${show_in_inventory} for ${result.length} items`,
    );
    ...
    return Response.json(
      { error: "حدث خطأ أثناء تحديث الأصناف", details: error.message },
      { status: 500 },
    );
  }
}
