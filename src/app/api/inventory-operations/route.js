...
export async function POST(request) {
  ...
  try {
    ...
    const unitSnapshots =
      await getDefaultInventoryUnitSnapshots(requestedItemIds);
    const insertItemIds = [];
    ...
    if (operation.inventory_type === "Transfer") {
      ...
      if (missingMoved.length > 0) {
        return Response.json(
          {
            error:
              "تعذّر حذف التحويل: كمية النقل المسجلة غير معروفة لبعض الأصناف",
      ...
      if (destBranchId && movedRows.length > 0) {
        ...
        if (conflicts.length > 0) {
          ...
          return Response.json(
            {
              error:
                "لا يمكن حذف هذا التحويل: توجد تحويلات لاحقة تعتمد على الكمية المنقولة. احذفها أولاً.",
      ...
      for (const pid of pairedIds) {
        ...
        txStatements.push(
          sql`DELETE FROM inventory_operations WHERE id = ${pid}`,
        );
      }
  ...
  try {
    ...
    async function replaceInventoryItems(opRowId, branchId, itemsList) {
      const snapshots = await getDefaultInventoryUnitSnapshots(
        itemsList.map((it) => it.itemId),
      );
      const statements = [
        sql`DELETE FROM inventory_items WHERE operation_id = ${opRowId}`,
      ];
    ...
    if (invType === "Transfer") {
      ...
      if (itemsProvided) {
        ...
        if (adjustments.length > 0) {
          const snapshots = await getDefaultInventoryUnitSnapshots(
            adjustments.map((adj) => adj.item_id),
          );
          const stmts = [];
          for (const adj of adjustments) {
            const { item_id, newMoved, delta, outNewQty, inNewQty } = adj;
    ...
    if (invType === "Opening") {
      ...
      if (sessions.length > 0) {
        ...
        const stmts = [
          sql`DELETE FROM opening_session_items WHERE session_id = ${sessionId}`,
        ];
        for (const it of cleanedItems) {
          const snap = snapshotForItem(snapshots, it.itemId);
          stmts.push(sql`
