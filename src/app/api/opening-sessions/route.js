...
export async function POST(request) {
  ...
  try {
    ...
    const { branchId, openedAt, note, items } = body;
    ...
    if (!Array.isArray(items) || items.length === 0) {
      return Response.json({ error: "قائمة الأصناف مطلوبة" }, { status: 400 });
    }
    ...
    for (const row of items) {
      ...
      cleaned.push({ itemId: itemIdNum, quantity: qtyNum });
    }
    ...
    const openedAtTimestamp = openedAt.includes("T")
      ? openedAt
      : `${openedAt}T00:00:00`;

    // Extract date-only for the unique constraint conflict check
    const openedAtDateOnly = openedAt.includes("T")
      ? openedAt.split("T")[0]
      : openedAt;

    // 1) Check if a session already exists for this branch+date
    const existingSessions = await sql(
      `SELECT id FROM opening_sessions 
       WHERE branch_id = $1 AND opened_at::date = $2::date`,
    ...
    if (existingSessions.length > 0) {
      ...
      const [updated] = await sql(
        `UPDATE opening_sessions 
         ...
         WHERE id = $3
         RETURNING id, branch_id, opened_at, note, created_at`,
        [openedAtTimestamp, note || null, existingSessions[0].id],
      );
      session = updated;
    } else {
      // Insert new session
      const [inserted] = await sql(
        `INSERT INTO opening_sessions (branch_id, opened_at, note)
         VALUES ($1, $2::timestamp, $3)
         RETURNING id, branch_id, opened_at, note, created_at`,
        [branchIdNum, openedAtTimestamp, note || null],
      );
      session = inserted;
    }
    ...
    for (const it of cleaned) {
      await sql(
        ...
        [session.id, it.itemId, it.quantity],
      );
    }
    ...
    const operationTimestamp = openedAt.includes("T")
      ? openedAt
      : `${openedAt}T00:00:00`;
    ...
    for (const it of cleaned) {
      await sql(
        `INSERT INTO inventory_items (operation_id, item_id, quantity, branch_id)
         VALUES ($1, $2, $3, $4)`,
        [operation.id, it.itemId, it.quantity, branchIdNum],
      );
    }
    ...
    return Response.json({ session, operation }, { status: 201 });
  } catch (error) {
