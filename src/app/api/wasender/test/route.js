...
export async function POST(request) {
  try {
    ...
    if (!to || !text) {
      return Response.json({ error: "to و text مطلوبة" }, { status: 400 });
    }

    const result = await sendWhatsAppViaWasender({ to, text });
    if (!result.ok) {
      return Response.json(
        { error: "فشل الإرسال", details: result.error, more: result.details },
        { status: 502 },
      );
    }
