...
export async function POST(request) {
  ...
  try {
    ...
    if (Array.isArray(body.items) && body.items.length > 0) {
      const { branchId, receivedAt, note, items } = body;
      ...
      for (const it of cleanedItems) {
        const [row] = await sql(
          `INSERT INTO purchase_receipts (branch_id, item_id, quantity, received_at, note, created_by_employee_id, created_by_employee_name, receipt_batch_id)
    ...
    const { branchId, itemId, quantity, receivedAt, note } = body;
    ...
    return Response.json({ receipt: row }, { status: 201 });
  } catch (error) {
...
export async function PUT(request) {
  ...
  try {
    ...
    return Response.json({ error: "فشل تعديل الوارد" }, { status: 500 });
  }
}
