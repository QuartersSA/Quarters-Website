        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-5 min-h-0">
            ...
            {selectedItem && (fromIdNum || toIdNum) ? (
              <div className="flex flex-wrap gap-2 items-center">
                {fromIdNum && fromBranchStock !== null ? (
                  <div className="flex items-center gap-2 px-3 py-2 rounded-2xl bg-slate-100 dark:bg-white/[0.04] border border-slate-200 dark:border-white/10">
                    <Package className="w-4 h-4 text-slate-500 dark:text-white/40" />
                    <span className="text-xs text-slate-600 dark:text-white/55">
                      مخزون المرسل{" "}
                      {stockIsPointInTime ? "بتاريخ التحويل" : "الحالي"}:
                    </span>
                    <span
                      className={`text-sm font-bold ${fromBranchStock > 0 ? "text-emerald-700 dark:text-emerald-300" : "text-red-700 dark:text-red-300"}`}
                    >
                      {fromBranchStock}
                    </span>
                  </div>
                ...
                {toIdNum && toBranchStock !== null ? (
                  <div className="flex items-center gap-2 px-3 py-2 rounded-2xl bg-slate-100 dark:bg-white/[0.04] border border-slate-200 dark:border-white/10">
                    ...
                    <span
                      className={`text-sm font-bold ${toBranchStock > 0 ? "text-emerald-700 dark:text-emerald-300" : "text-amber-700 dark:text-amber-300"}`}
                    >
                      {toBranchStock}
                    </span>
                  </div>
          ...
          <div className={`${ws.glassSoft} ${ws.card} p-4`}>
            <div className="flex items-center justify-between mb-3">
              <h4 className="text-slate-900 dark:text-white font-bold">الأصناف المحددة</h4>
              ...
              <div className="space-y-2">
                {items.map((it) => {
                  ...
                  return (
                    <div
                      key={it.itemId}
                      className="p-3 rounded-2xl bg-slate-100 dark:bg-white/[0.04] border border-slate-200 dark:border-white/10 space-y-2"
                    >
                      <div className="flex items-center justify-between gap-3">
                        ...
                        <div className="flex items-center gap-2">
                          <span
                            className={`${ws.pill} bg-slate-100 dark:bg-white/[0.06] text-slate-900 dark:text-white border-slate-200 dark:border-white/10`}
                          >
                            {it.quantity}
                          </span>
                          <button
