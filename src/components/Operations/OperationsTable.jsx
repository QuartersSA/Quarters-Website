...
export function OperationsTable({
  ...
  const headerChip =
    "px-3 py-1 rounded-full text-sm text-slate-700 dark:text-slate-700 dark:text-white/70 bg-slate-100 dark:bg-slate-100 dark:bg-white/[0.04] border border-slate-200 dark:border-slate-200 dark:border-white/10";
  ...
  return (
    <div className={cardClass} dir="rtl">
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            ...
            <div className="text-xs text-slate-500 dark:text-slate-500 dark:text-white/50">عرض وتصدير السجل</div>
          </div>
          ...
          <div>
            ...
            >
              <button
                ...
                className="w-full flex items-center gap-3 px-4 py-3 text-right text-slate-800 dark:text-slate-800 dark:text-white/85 hover:bg-slate-100 dark:hover:bg-slate-100 dark:hover:bg-white/[0.06] transition-colors"
              >
                <FileText className="w-4 h-4 text-emerald-700 dark:text-emerald-700 dark:text-emerald-200" />
                <div>
                  <p className="font-semibold text-slate-900 dark:text-slate-900 dark:text-white">Excel</p>
                  <p className="text-xs text-slate-500 dark:text-slate-500 dark:text-white/45">ملف .xls</p>
                </div>
              </button>
              <button
                type="button"
                onClick={handleExportPDF}
                className="w-full flex items-center gap-3 px-4 py-3 text-right text-slate-800 dark:text-slate-800 dark:text-white/85 hover:bg-slate-100 dark:hover:bg-slate-100 dark:hover:bg-white/[0.06] transition-colors border-t border-slate-200 dark:border-slate-200 dark:border-white/10"
              >
                <FileText className="w-4 h-4 text-red-700 dark:text-red-700 dark:text-red-200" />
                <div>
                  <p className="font-semibold text-slate-900 dark:text-slate-900 dark:text-white">PDF</p>
                  <p className="text-xs text-slate-500 dark:text-slate-500 dark:text-white/45">للطباعة والأرشفة</p>
                </div>
              </button>
            </GlassPopover>
          </div>
      ...
      {selectionEnabled && selectedCount > 0 ? (
        ...
        >
          <div className="flex items-center gap-3 text-sm text-slate-900 dark:text-slate-900 dark:text-white">
            <CheckSquare className="w-4 h-4 text-emerald-700 dark:text-emerald-700 dark:text-emerald-300" />
            <span>
              تم تحديد <strong className="text-emerald-700 dark:text-emerald-700 dark:text-emerald-300">{selectedCount}</strong> عملية
            </span>
            <button
              type="button"
              onClick={onClearSelection}
              className="text-slate-600 dark:text-slate-600 dark:text-white/55 hover:text-slate-900 dark:hover:text-slate-900 dark:hover:text-white text-xs underline-offset-2 hover:underline"
            >
              إلغاء التحديد
            </button>
          </div>
      ...
      <div className="md:hidden divide-y divide-slate-200 dark:divide-white/10">
        {isLoading ? (
          <div className="px-6 py-12 text-center text-slate-600 dark:text-white/55">
          ...
          filteredOperations.map((operation) => {
            ...
            const transferParties = isTransfer
              ...
              : null;
            const TransferIcon = transferParties?.Icon;
            ...
            return (
              <div
                ...
                className={`p-4 transition-colors ${
                  isSelected ? "bg-emerald-400/[0.06]" : ""
                }`}
              >
                  ...
                  <div className="flex items-center gap-3 min-w-0">
                    {selectionEnabled ? (
                      <button
                        ...
                        className="inline-flex items-center justify-center w-8 h-8 rounded-xl bg-slate-100 dark:bg-white/[0.04] border border-slate-200 dark:border-white/10 disabled:opacity-40"
                        aria-label={isSelected ? "إلغاء التحديد" : "تحديد"}
                      >
                        {isSelected ? (
                          <CheckSquare className="w-4 h-4 text-emerald-700 dark:text-emerald-300" />
                        ) : (
                          <Square className="w-4 h-4 text-slate-500 dark:text-white/40" />
                        )}
                      </button>
                    ) : null}
                    <div className="min-w-0">
                      <div className="flex items-center gap-2">
                        <FileText className="w-4 h-4 text-emerald-700 dark:text-emerald-200 shrink-0" />
                        <p className="text-slate-900 dark:text-white font-mono font-semibold truncate">
                          {operation.inventory_number}
                        </p>
                      </div>
                      <p className="mt-1 text-xs text-slate-500 dark:text-white/45 truncate">
                        {operation.employee_name || "غير محدد"}
                      </p>
                    </div>
                  </div>
                  <span
                    className={`${ws.pill} ${typeMeta.className} inline-flex items-center gap-1 shrink-0`}
                  >
                ...
                <div className="mt-3 rounded-xl bg-slate-100 dark:bg-white/[0.04] px-3 py-2">
                  {isTransfer && transferParties ? (
                    <div className="space-y-1 text-sm">
                      <div className="text-slate-800 dark:text-white/80 font-medium flex items-center gap-1">
                        {TransferIcon ? (
                          <TransferIcon className="w-3.5 h-3.5 text-emerald-700 dark:text-emerald-200" />
                        ) : null}
                    ...
                    <div className="flex items-center gap-2 text-sm text-slate-800 dark:text-white/80">
                      <Building2 className="w-4 h-4 text-slate-500 dark:text-white/35" />
                ...
                <div className="mt-3 grid grid-cols-2 gap-2 text-sm">
                  <div className="rounded-xl bg-slate-100 dark:bg-white/[0.04] px-3 py-2">
                    <p className="text-[11px] text-slate-500 dark:text-white/45">تاريخ العملية</p>
                  ...
                  <div className="rounded-xl bg-slate-100 dark:bg-white/[0.04] px-3 py-2">
                    <p className="text-[11px] text-slate-500 dark:text-white/45">تاريخ الإدخال</p>
                    <p className="font-semibold text-slate-900 dark:text-white">
                ...
                <div className="mt-3 flex items-center gap-2">
                  <button
                    ...
                    className={`${ws.btnPrimary} flex-1 px-3 py-2 text-sm justify-center`}
                  >
        ...
        <table className="w-full">
          <thead>
            <tr className="bg-slate-100 dark:bg-slate-100 dark:bg-white/[0.04]">
              {selectionEnabled ? (
                <th className="px-4 py-4 text-center" style={{ width: 48 }}>
                  <button
                    ...
                    className="inline-flex items-center justify-center w-6 h-6 rounded-md hover:bg-slate-100 dark:hover:bg-slate-100 dark:hover:bg-white/[0.06] transition-colors"
                  ...
                  >
                    {allSelectedOnPage ? (
                      <CheckSquare className="w-5 h-5 text-emerald-700 dark:text-emerald-700 dark:text-emerald-300" />
                    ) : (
                      <Square className="w-5 h-5 text-slate-500 dark:text-slate-500 dark:text-white/40" />
                    )}
                  </button>
                </th>
              ) : null}
              ...
              <th className="text-center px-6 py-4 text-sm font-semibold text-slate-600 dark:text-slate-600 dark:text-white/55">
                الإجراءات
              </th>
            </tr>
          </thead>
            ...
            {isLoading ? (
              <tr>
                <td
                  ...
                  className="px-6 py-12 text-center text-slate-600 dark:text-slate-600 dark:text-white/55"
                >
                  <div className="flex items-center justify-center gap-3">
                    <div className="w-6 h-6 border-2 border-emerald-400/60 border-t-transparent rounded-full animate-spin" />
                    <span>جاري التحميل…</span>
              ...
              filteredOperations.map((operation) => {
                ...
                return (
                  <tr
                    key={operation.id}
                    className={`border-t border-slate-100 dark:border-slate-100 dark:border-white/5 transition-colors ${
                      isSelected
                        ? "bg-emerald-400/[0.06] hover:bg-emerald-400/[0.10]"
                        : "hover:bg-slate-100 dark:hover:bg-slate-100 dark:hover:bg-white/[0.05]"
                    }`}
                  >
                    {selectionEnabled ? (
                      <td className="px-4 py-4 text-center" style={{ width: 48 }}>
                        {isSelectable ? (
                          ...
                          <span
                            className="text-slate-300 dark:text-slate-300 dark:text-white/20 text-xs"
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        <FileText className="w-4 h-4 text-emerald-700 dark:text-emerald-700 dark:text-emerald-200" />
                        <span className="text-slate-900 dark:text-slate-900 dark:text-white font-mono text-sm">
                          {operation.inventory_number}
                        </span>
                      </div>
                    </td>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        <Building2 className="w-4 h-4 text-slate-400 dark:text-slate-400 dark:text-white/35" />
                        <div className="min-w-0">
                          {isTransfer && transferParties ? (
                            ...
                            <>
                              <div className="text-slate-800 dark:text-white/80 font-medium truncate flex items-center gap-1">
                                <transferParties.Icon className="w-3.5 h-3.5 text-emerald-700 dark:text-emerald-700 dark:text-emerald-200" />
                                <span>
                                  المستلم: {transferParties.receiver}
                                </span>
                              </div>
                              <div className="text-slate-600 dark:text-slate-600 dark:text-white/55 text-xs mt-1 truncate">
                                المرسل: {transferParties.sender}
                              </div>
                            </>
                          ) : (
                            <>
                              ...
                              {operation.branch_location ? (
                                <div className="text-slate-500 dark:text-slate-500 dark:text-white/40 text-xs">
                                  {operation.branch_location}
                                </div>
                              ) : null}
                            </>
                    ...
                    <td className="px-6 py-4">
                      {operation.employee_name ? (
                        <div className="flex items-center gap-2">
                          <div className="w-8 h-8 bg-slate-100 dark:bg-slate-100 dark:bg-white/[0.06] border border-slate-200 dark:border-slate-200 dark:border-white/10 rounded-full flex items-center justify-center flex-shrink-0">
                        ...
                        <span className="text-slate-500 dark:text-slate-500 dark:text-white/45 text-sm">غير محدد</span>
                      )}
                    </td>
                    ...
                    <td className="px-6 py-4">
                      <div className="text-sm">
                        <div className="text-slate-900 dark:text-slate-900 dark:text-white">
                          {renderDateRiyadh(operationDateValue)}
                        </div>
                        <div className="text-slate-500 dark:text-slate-500 dark:text-white/45 text-xs">
                          {renderTimeRiyadh(operationDateValue)}
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="px-6 py-4">
                      <div className="text-sm">
                        <div className="text-slate-600 dark:text-slate-600 dark:text-white/60">
                          {renderDateRiyadh(operation.created_at)}
                        </div>
                        <div className="text-slate-400 dark:text-slate-400 dark:text-white/35 text-xs">
                          {renderTimeRiyadh(operation.created_at)}
                        </div>
                      </div>
                    </td>
                      ...
                      <div className="flex items-center justify-center gap-2">
                        ...
                        {onEditOperation ? (
                          <button
                            ...
                            title="تعديل العملية"
                          >
                            <Pencil className="w-4 h-4 text-sky-700 dark:text-sky-700 dark:text-sky-200" />
                          </button>
                        ) : null}
                        <button
                          type="button"
              ...
              <tr>
                <td
                  colSpan={selectionEnabled ? "8" : "7"}
                  className="px-6 py-12 text-center text-slate-500 dark:text-slate-500 dark:text-white/45"
                >
                  <ClipboardList className="w-12 h-12 mx-auto mb-3 opacity-40" />
                  <p className="text-lg text-slate-700 dark:text-slate-700 dark:text-white/70">لا توجد عمليات جرد</p>
                  {hasActiveFilters ? (
                    <p className="text-sm mt-2">جرّب تغيير الفلاتر أو مسحها</p>
                  ) : null}
                </td>
