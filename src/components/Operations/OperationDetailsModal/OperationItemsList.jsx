...
export function OperationItemsList({ operationDetails }) {
  ...
  return (
    <div dir="rtl">
      <h4 className="text-lg font-bold text-slate-900 dark:text-slate-900 dark:text-white mb-4 flex items-center gap-3 tracking-tight">
      ...
      {operationDetails?.items ? (
          ...
          {operationDetails.items.map((item, idx) => {
            ...
            const badgeClass =
              useAvailabilityChrome && isZero
                ? `${ws.pill} bg-red-500/10 text-red-700 dark:text-red-700 dark:text-red-200 border-red-500/20`
                : useAvailabilityChrome
                  ? `${ws.pill} bg-emerald-500/10 text-emerald-700 dark:text-emerald-700 dark:text-emerald-200 border-emerald-500/20`
                  : `${ws.pill} bg-slate-100 dark:bg-slate-100 dark:bg-white/[0.06] text-slate-700 dark:text-slate-700 dark:text-white/70 border-slate-200 dark:border-slate-200 dark:border-white/10`;
            ...
            const iconBg =
              useAvailabilityChrome && isZero
                ...
                : useAvailabilityChrome
                  ? "bg-emerald-500/10"
                  : "bg-slate-100 dark:bg-slate-100 dark:bg-white/[0.04]";
            ...
            const iconColor =
              useAvailabilityChrome && isZero
                ...
                : useAvailabilityChrome
                  ? "text-emerald-700 dark:text-emerald-700 dark:text-emerald-200"
                  : "text-slate-700 dark:text-slate-700 dark:text-white/70";
            ...
            return (
              <div
                ...
                className={`${ws.glassSoft} border border-slate-200 dark:border-slate-200 dark:border-white/10 rounded-3xl p-4 flex items-center justify-between hover:bg-slate-100 dark:hover:bg-slate-100 dark:hover:bg-white/[0.06] transition-colors`}
              >
                <div className="flex items-center gap-3 flex-1">
                  <div
                    className={`w-10 h-10 rounded-2xl flex items-center justify-center border border-slate-200 dark:border-slate-200 dark:border-white/10 ${iconBg}`}
                  >
        ...
        <div className="text-center py-8 text-slate-600 dark:text-slate-600 dark:text-white/55">
