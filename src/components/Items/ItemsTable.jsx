...
export function ItemsTable({
  ...
  const headerCell =
    "text-right px-6 py-4 text-sm font-semibold text-slate-600 dark:text-slate-600 dark:text-slate-600 dark:text-white/55 whitespace-nowrap";
  const colCount = 10;
  ...
  const checkboxActive =
    "bg-emerald-400/20 border border-emerald-400/40 text-emerald-700 dark:text-emerald-700 dark:text-emerald-300";
  const checkboxInactive =
    "bg-slate-100 dark:bg-slate-100 dark:bg-slate-100 dark:bg-white/[0.04] border border-slate-200 dark:border-slate-200 dark:border-slate-200 dark:border-white/15 text-slate-400 dark:text-slate-400 dark:text-slate-400 dark:text-white/30 hover:bg-slate-200 dark:hover:bg-slate-200 dark:hover:bg-slate-200 dark:hover:bg-white/[0.07]";
  const checkboxPartial =
    "bg-emerald-400/10 border border-emerald-400/30 text-emerald-700 dark:text-emerald-700 dark:text-emerald-300";
  ...
  return (
    <>
            ...
            <tbody>
              {items.map((item) => {
                ...
                const statusPillClass = !showInInventory
                  ? `${ws.pill} bg-red-500/15 text-red-700 dark:text-red-700 dark:text-red-200 border-red-500/25`
                  : stockStatus === "out_of_stock"
                    ? `${ws.pill} bg-red-500/15 text-red-700 dark:text-red-700 dark:text-red-200 border-red-500/25`
                    : stockStatus === "low_stock"
                      ? `${ws.pill} bg-amber-500/15 text-amber-700 dark:text-amber-700 dark:text-amber-200 border-amber-500/25`
                      : `${ws.pill} bg-emerald-500/15 text-emerald-700 dark:text-emerald-700 dark:text-emerald-200 border-emerald-500/25`;
                ...
                const rowBg = isChecked
                  ? "bg-emerald-400/[0.04]"
                  : "hover:bg-slate-100 dark:hover:bg-slate-100 dark:hover:bg-slate-100 dark:hover:bg-white/[0.05]";

                return (
                  <tr
                    key={item.id}
                    className={`border-t border-slate-100 dark:border-slate-100 dark:border-slate-100 dark:border-white/5 transition-colors ${rowBg}`}
                  >
                    {/* Checkbox */}
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3 min-w-[240px]">
                        ...
                        <div className="min-w-0">
                          <div className="text-slate-900 dark:text-slate-900 dark:text-slate-900 dark:text-white font-medium truncate">
                            {item?.name || "-"}
                          </div>
                          {item?.name_en ? (
                            <div
                              className="text-slate-500 dark:text-slate-500 dark:text-slate-500 dark:text-white/45 text-xs truncate"
                              dir="ltr"
                            >
                              {item.name_en}
                            </div>
                    ...
                    {/* Description */}
                    <td className="px-6 py-4 text-slate-700 dark:text-slate-700 dark:text-slate-700 dark:text-white/70 text-sm">
                      <div
                    ...
                    <td className="px-6 py-4">
                      <span
                        className={`${ws.pill} bg-slate-50 dark:bg-slate-50 dark:bg-slate-50 dark:bg-white/[0.03] text-slate-700 dark:text-slate-700 dark:text-slate-700 dark:text-white/70 border-slate-200 dark:border-slate-200 dark:border-slate-200 dark:border-white/10 inline-flex items-center gap-2`}
                      >
                    ...
                    {/* Unit */}
                    <td className="px-6 py-4 text-slate-700 dark:text-slate-700 dark:text-slate-700 dark:text-white/75 whitespace-nowrap">
                      <span
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      ...
                      {item?.linked_green_bean_name ? (
                        <div className="flex items-center gap-1 mt-1">
                          <Link className="w-3 h-3 text-amber-700 dark:text-amber-700 dark:text-amber-300/50" />
                          <span className="text-amber-700 dark:text-amber-700 dark:text-amber-200/50 text-[10px]">
                            {item.linked_green_bean_name}
                    ...
                    {/* Stock */}
                    <td className="px-6 py-4 text-slate-800 dark:text-slate-800 dark:text-slate-800 dark:text-white/85 whitespace-nowrap">
                      {stockText}
                    </td>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {typeof onManageBranches === "function" ? (
                          <button
                            ...
                            className={`${ws.iconButton} text-purple-700 dark:text-purple-700 dark:text-purple-200`}
      ...
      {selectedCount > 0 ? (
        ...
        >
          <div
            className={`${ws.popover} border border-slate-200 dark:border-slate-200 dark:border-slate-200 dark:border-white/15 rounded-2xl px-5 py-3.5 flex items-center gap-4 shadow-2xl`}
          >
            {/* Counter */}
            <div className="flex items-center gap-2 text-slate-900 dark:text-slate-900 dark:text-slate-900 dark:text-white font-semibold whitespace-nowrap">
              <CheckSquare className="w-5 h-5 text-emerald-700 dark:text-emerald-700 dark:text-emerald-300" />
              <span>{selectedCount}</span>
              <span className="text-slate-500 dark:text-slate-500 dark:text-slate-500 dark:text-white/50 text-sm font-normal">محدد</span>
            </div>

            <div className="w-px h-7 bg-slate-200 dark:bg-slate-200 dark:bg-slate-200 dark:bg-white/10" />
            ...
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 rounded-xl bg-emerald-400/15 text-emerald-700 dark:text-emerald-700 dark:text-emerald-200 border border-emerald-400/25 font-semibold hover:bg-emerald-400/25 transition-colors disabled:opacity-50 whitespace-nowrap"
            >
              <ClipboardCheck className="w-4 h-4" />
            ...
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 rounded-xl bg-red-500/15 text-red-700 dark:text-red-700 dark:text-red-200 border border-red-500/25 font-semibold hover:bg-red-500/25 transition-colors disabled:opacity-50 whitespace-nowrap"
            >
