            ...
            {item.branch_stock?.map((stock) => {
              ...
              const rowClass = isOutOfStock
                ...
                : isLow
                  ...
                  : "bg-slate-100 dark:bg-slate-100 dark:bg-slate-100 dark:bg-white/[0.04] border-slate-200 dark:border-slate-200 dark:border-slate-200 dark:border-white/10";
              ...
              const iconClass = isOutOfStock
                ...
                : isLow
                  ...
                  : "text-sky-700 dark:text-sky-700 dark:text-sky-200";

              const qtyClass = isOutOfStock
                ? "text-red-700 dark:text-red-700 dark:text-red-200"
                : isLow
                  ? "text-amber-700 dark:text-amber-700 dark:text-amber-200"
                  : "text-slate-900 dark:text-slate-900 dark:text-slate-900 dark:text-white";

              const statusLabel = isOutOfStock ? (
                <p className="text-red-700 dark:text-red-700 dark:text-red-200 text-xs flex items-center gap-1">
                  <XCircle className="w-3 h-3" />
                  نفد المخزون
                </p>
              ) : isLow ? (
                <p className="text-amber-700 dark:text-amber-700 dark:text-amber-200 text-xs flex items-center gap-1">
                  <AlertTriangle className="w-3 h-3" />
                  أقل من الحد الأدنى
                </p>
              ) : (
                <p className="text-emerald-700 dark:text-emerald-700 dark:text-emerald-200 text-xs flex items-center gap-1">
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-slate-900 dark:text-slate-900 dark:text-slate-900 dark:text-white font-medium">
                        {stock.branch_name}
                      </p>
                      {statusLabel}
                    </div>
                  ...
                  <div className="text-left">
                    ...
                    <p className="text-slate-500 dark:text-slate-500 dark:text-slate-500 dark:text-white/45 text-xs">
                      {item.unit || "وحدة"}
                    </p>
                  </div>
                </div>
