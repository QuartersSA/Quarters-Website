    ...
    >
      ...
      >
        ...
        <div className="p-6 overflow-y-auto flex-1">
          <form onSubmit={onCreate} className="space-y-3">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              <div className="relative">
                <Languages className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400 dark:text-slate-400 dark:dark:text-slate-400 dark:dark:dark:text-white/35" />
                <input
                  value={nameEn}
          ...
          {localError ? (
            <div className="mt-3 p-3 bg-red-500/10 border border-red-500/25 rounded-2xl text-red-700 dark:text-red-700 dark:dark:text-red-200 text-sm">
              {localError}
            </div>
          ) : null}

          <div className="mt-6">
            <div className="text-sm text-slate-600 dark:text-slate-600 dark:dark:text-slate-600 dark:dark:dark:text-white/55 mb-3">
              الفئات ({sorted.length})
            </div>

            {sorted.length === 0 ? (
              <div className="p-6 text-center text-slate-500 dark:text-slate-500 dark:dark:text-slate-500 dark:dark:dark:text-white/45 border border-slate-200 dark:border-slate-200 dark:dark:border-slate-200 dark:dark:dark:border-white/10 rounded-3xl bg-slate-50 dark:bg-slate-50 dark:dark:bg-slate-50 dark:dark:dark:bg-white/[0.03]">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {sorted.map((c) => {
                  ...
                  return (
                    <div
                      key={c.id}
                      className={`${ws.glassSoft} border border-slate-200 dark:border-slate-200 dark:dark:border-slate-200 dark:dark:dark:border-white/10 rounded-2xl px-4 py-3`}
                    >
                      {!isEditing ? (
                        <div className="flex items-start justify-between gap-3">
                          <div className="min-w-0">
                            <div className="text-slate-900 dark:text-white/90 font-semibold truncate">
                              {c.name}
                            </div>
                            <div
                              className="text-slate-600 dark:text-slate-600 dark:dark:text-slate-600 dark:dark:dark:text-white/55 text-sm mt-1 truncate"
                              dir="ltr"
                            >
                              {c.name_en || "-"}
                            </div>
            ...
            <div className="mt-5 text-xs text-slate-500 dark:text-slate-500 dark:dark:text-slate-500 dark:dark:dark:text-white/40">
