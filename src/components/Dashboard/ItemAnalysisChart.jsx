...
export function ItemAnalysisChart({
  ...
  if (!analysisItemId) {
    chartBody = (
      <div className="text-center text-slate-600 dark:text-slate-600 dark:text-white/55 py-10">
    ...
    chartBody = (
      <div className="text-center text-red-700 dark:text-red-700 dark:text-red-200 py-10">
        ...
        <div className="flex flex-wrap gap-x-6 gap-y-1 mt-3">
          {activeSeries.map((s) => (
            <div key={s.dataKey} className="flex items-center gap-2">
              ...
              />
              <span className="text-xs text-slate-500 dark:text-slate-500 dark:text-white/50">{s.name}</span>
            </div>
          ))}
        </div>

        <p className="text-xs text-slate-500 dark:text-slate-500 dark:text-white/40 mt-3">
        ...
        <div className="flex flex-col xl:flex-row xl:items-end xl:justify-between gap-4">
          <div>
            <h2 className="text-xl font-bold text-slate-900 dark:text-slate-900 dark:text-white flex items-center gap-3 tracking-tight">
              <div className={`${ws.iconBox} w-10 h-10 text-violet-700 dark:text-violet-700 dark:text-violet-200`}>
                <Layers className="w-5 h-5" />
              </div>
              {title}
            </h2>
            {subtitle ? (
              <p className="text-slate-500 dark:text-slate-500 dark:text-white/45 text-sm mt-1">{subtitle}</p>
            ) : null}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-3 w-full xl:w-auto">
            ...
            <div className="w-full">
              <label className="block text-xs text-slate-500 dark:text-slate-500 dark:text-white/45 mb-1">من</label>
              <GlassDatePicker
                value={analysisFrom}
            ...
            <div className="w-full">
              <label className="block text-xs text-slate-500 dark:text-slate-500 dark:text-white/45 mb-1">إلى</label>
              <GlassDatePicker
                value={analysisTo}
