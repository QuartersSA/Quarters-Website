  ...
  return (
    ...
    >
      <div className={cardClass}>
        <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-4 flex items-center gap-3 tracking-tight">
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <span className="text-slate-600 dark:text-white/55">إجمالي الأصناف</span>
            <span className="text-2xl font-bold text-slate-900 dark:text-white">
              {items?.length || 0}
            </span>
          </div>
          <div className="flex items-center justify-between">
            <span className="text-slate-600 dark:text-white/55">الأصناف النشطة</span>
      ...
      <div className={cardClass}>
        ...
        <div className="space-y-2">
          {branches?.slice(0, 3).map((branch) => (
            ...
            >
              <div>
                <p className="text-slate-900 dark:text-white font-medium">{branch.name}</p>
                <p className="text-slate-500 dark:text-white/45 text-sm">{branch.location}</p>
              </div>
              <CheckCircle className="w-5 h-5 text-emerald-700 dark:text-emerald-200" />
            </div>
          ))}
        </div>
      </div>
