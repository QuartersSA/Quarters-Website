...
function getBranchHealthColor(lowStock, outOfStock, tracked) {
  if (tracked === 0)
    return { dot: "bg-gray-400", label: "لا بيانات", color: "text-white/40" };
  const ratio = (lowStock + outOfStock * 2) / Math.max(tracked, 1);
  if (ratio === 0)
    return { dot: "bg-emerald-400", label: "ممتاز", color: "text-emerald-200" };
  if (ratio < 0.15)
    return { dot: "bg-emerald-400", label: "جيد", color: "text-emerald-200" };
  if (ratio < 0.3)
    return {
      dot: "bg-amber-400",
      label: "تحتاج متابعة",
  ...
  return { dot: "bg-red-400", label: "حالة حرجة", color: "text-red-200" };
}
...
export function BranchPerformanceCard({ branchPerformance, weekComparison }) {
  ...
  const changeColor =
    weekComparison?.changePercent >= 0 ? "text-emerald-200" : "text-red-200";
      ...
      <div className={`${ws.glass} ${ws.card} p-6`}>
        ...
        <div className="grid grid-cols-3 gap-4 mb-4">
          <div className={`${ws.glassSoft} rounded-2xl p-4 text-center`}>
            <p className="text-white/50 text-xs mb-1">هذا الأسبوع</p>
            <p className="text-2xl font-bold text-white">
              {weekComparison?.thisWeek || 0}
            </p>
          </div>
          <div className={`${ws.glassSoft} rounded-2xl p-4 text-center`}>
            <p className="text-white/50 text-xs mb-1">الأسبوع الماضي</p>
            <p className="text-2xl font-bold text-white">
              {weekComparison?.lastWeek || 0}
            </p>
          </div>
          <div className={`${ws.glassSoft} rounded-2xl p-4 text-center`}>
            <p className="text-white/50 text-xs mb-1">التغيير</p>
