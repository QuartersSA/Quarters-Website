...
function getDeltaClass(delta) {
  ...
  if (n < -0.001) return "text-red-700 dark:text-red-200";
  if (n > 0.001) return "text-emerald-700 dark:text-emerald-200";
      ...
      {!hasFilters ? (
        <div className="p-12 text-center text-slate-500 dark:text-white/50">
          ...
          <div className="mt-4 inline-flex flex-col gap-1.5 items-start text-sm">
            <span className={f.branch ? "text-emerald-700 dark:text-emerald-300" : "text-slate-600 dark:text-white/55"}>
              {f.branch ? "✓" : "○"} الفرع
            </span>
            <span className={f.item ? "text-emerald-700 dark:text-emerald-300" : "text-slate-600 dark:text-white/55"}>
              {f.item ? "✓" : "○"} الصنف
            </span>
            <span className={f.from ? "text-emerald-700 dark:text-emerald-300" : "text-slate-600 dark:text-white/55"}>
              {f.from ? "✓" : "○"} من تاريخ
            </span>
            <span className={f.to ? "text-emerald-700 dark:text-emerald-300" : "text-slate-600 dark:text-white/55"}>
              {f.to ? "✓" : "○"} إلى تاريخ
            </span>
          </div>
            ...
            <tbody>
              {rows.map((r) => {
                ...
                return (
                  ...
                  >
                    <td className="px-4 py-3 text-slate-700 dark:text-white/75 text-sm whitespace-nowrap" dir="ltr">
                      {formatDate(r.created_at)}
                    </td>
                    <td className="px-4 py-3 text-slate-600 dark:text-white/55 text-sm font-mono">
                      {r.inventory_number || "—"}
                    </td>
                    <td className="px-4 py-3 text-slate-900 dark:text-white text-sm font-semibold" dir="ltr">
