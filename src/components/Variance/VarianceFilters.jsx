      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-3">
        ...
        <div>
          ...
          <div className="relative">
            <Search className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400 dark:text-white/35 z-10 pointer-events-none" />
            <input
              type="text"
              value={itemSearch}
          ...
          {hasSearch ? (
            <p
              className={`mt-1 text-xs ${
                matchCount === 0 ? "text-red-700 dark:text-red-200" : "text-slate-500 dark:text-white/45"
              }`}
            >
              {matchCount === 0
                ? "لا توجد نتائج"
