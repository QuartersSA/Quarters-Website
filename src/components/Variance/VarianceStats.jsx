  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 mb-8">
      <StatCard
        ...
        iconColor="text-sky-700 dark:text-sky-700 dark:dark:text-sky-200"
        label="عدد عمليات الجرد"
        value={stats.count}
      />
      <StatCard
        icon={<TrendingDown className="w-6 h-6" />}
        iconColor="text-red-700 dark:text-red-700 dark:dark:text-red-200"
        label="مجموع الفاقد"
        value={formatNumber(-stats.totalLoss)}
        valueColor={stats.totalLoss > 0 ? "text-red-700 dark:text-red-700 dark:dark:text-red-200" : "text-slate-900 dark:text-slate-900 dark:dark:text-white"}
      />
      <StatCard
        icon={<TrendingUp className="w-6 h-6" />}
        iconColor="text-emerald-700 dark:text-emerald-700 dark:dark:text-emerald-200"
        label="مجموع الزيادة"
        value={formatNumber(stats.totalGain)}
        valueColor={stats.totalGain > 0 ? "text-emerald-700 dark:text-emerald-700 dark:dark:text-emerald-200" : "text-slate-900 dark:text-slate-900 dark:dark:text-white"}
      />
      <StatCard
        icon={<AlertTriangle className="w-6 h-6" />}
        iconColor="text-amber-700 dark:text-amber-700 dark:dark:text-amber-200"
...
function StatCard({ icon, iconColor, label, value, valueColor, sublabel }) {
  return (
    <div className={statCard}>
      ...
      <p
        className={`text-3xl font-bold tracking-tight ${valueColor || "text-slate-900 dark:text-slate-900 dark:dark:text-white"}`}
        dir="ltr"
      >
        {value}
      </p>
      {sublabel ? (
        <p className="text-slate-500 dark:text-slate-500 dark:dark:text-white/40 text-xs mt-1">{sublabel}</p>
      ) : null}
    </div>
  );
}
