        ...
        <table className="w-full">
          ...
          <tbody>
            {employees.map((employee) => {
              const rolePillClass =
                employee.role === "Admin"
                  ? `${ws.pill} bg-fuchsia-500/10 text-fuchsia-700 dark:text-fuchsia-200 border-fuchsia-500/20`
                  : `${ws.pill} bg-sky-500/10 text-sky-700 dark:text-sky-200 border-sky-500/20`;

              const workspaceBtnClass = employee.can_access_workspace
                ? "bg-emerald-500/15 text-emerald-700 dark:text-emerald-200 border-emerald-500/25 hover:bg-emerald-500/20"
                : "bg-slate-100 dark:bg-slate-100 dark:dark:bg-white/[0.04] text-slate-600 dark:text-slate-600 dark:dark:text-white/60 border-slate-200 dark:border-slate-200 dark:dark:border-white/10 hover:bg-slate-100 dark:hover:bg-slate-100 dark:dark:hover:bg-white/[0.06]";
                ...
                >
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      <div className={`${ws.iconBox} w-10 h-10 text-slate-800 dark:text-slate-800 dark:dark:text-white/85`}>
                        <User className="w-5 h-5" />
                      </div>
                      <span className="text-slate-900 dark:text-slate-900 dark:dark:text-white font-medium">
                        {employee.name}
                      </span>
                    </div>
                  </td>
                  ...
                  <td className="px-6 py-4 text-slate-700 dark:text-slate-700 dark:dark:text-white/75" dir="ltr">
                    {employee.phone || "-"}
                  </td>
                  <td className="px-6 py-4 text-slate-600 dark:text-slate-600 dark:dark:text-white/55">
                    {employee.email || "-"}
                  </td>
                  <td className="px-6 py-4">
                    <span
                  ...
                  <td className="px-6 py-4">
                    {tasks.length > 0 ? (
                      <div className="flex flex-wrap gap-1">
                        {tasks.map((t) => {
                          ...
                          return (
                            <span
                              key={t}
                              className={`${ws.pill} bg-slate-50 dark:bg-slate-50 dark:dark:bg-white/[0.03] text-slate-700 dark:text-slate-700 dark:dark:text-white/70 border-slate-200 dark:border-slate-200 dark:dark:border-white/10 inline-flex items-center gap-1`}
                            >
                              {icon}
                              <span>{t}</span>
                            </span>
                      ...
                      <span className="text-slate-500 dark:text-slate-500 dark:dark:text-white/45 text-sm">—</span>
                    )}
                  </td>
                  <td className="px-6 py-4">
                    {employee.branches && employee.branches.length > 0 ? (
                      ...
                      <span className="text-slate-500 dark:text-slate-500 dark:dark:text-white/45 text-sm">
                        لا يوجد فروع
                      </span>
                    )}
                  </td>
