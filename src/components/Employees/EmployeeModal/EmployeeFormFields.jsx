...
export function EmployeeFormFields({
  ...
  return (
    <div className="space-y-6">
      {/* Name */}
      <div>
        <label className="block text-sm font-semibold text-slate-700 dark:text-slate-700 dark:dark:text-white/70 mb-2">
      ...
      <div>
        ...
        <input
          ...
          dir="ltr"
        />
        <p className="text-xs text-slate-500 dark:text-slate-500 dark:dark:text-white/45 mt-2">
          ...
          <button
            ...
            className={`p-4 rounded-2xl border transition-colors ${
              formData.role === "Admin"
                ...
                : `${ws.glassSoft} text-slate-600 dark:text-slate-600 dark:dark:text-white/60 hover:bg-slate-100 dark:hover:bg-slate-100 dark:dark:hover:bg-white/[0.06]`
            }`}
          >
        ...
        >
          <div className="flex items-start justify-between gap-4 flex-wrap">
            <div className="min-w-0">
              <p className="text-slate-900 dark:text-slate-900 dark:dark:text-white font-semibold">صلاحيات أقسام الإدارة</p>
              <p className="text-sm text-slate-500 dark:text-slate-500 dark:dark:text-white/50 mt-1">
      ...
      {showAdminOptions ? (
        ...
        >
          <div className="flex items-start justify-between gap-4 flex-wrap">
            <div className="min-w-0">
              <p className="text-slate-900 dark:text-slate-900 dark:dark:text-white font-semibold">إشعارات واتساب</p>
              <p className="text-sm text-slate-500 dark:text-slate-500 dark:dark:text-white/50 mt-1">
      ...
      {showEmployeeOptions ? (
        ...
        >
          <div className="flex items-start justify-between gap-4 flex-wrap">
            <div className="min-w-0">
              <p className="text-slate-900 dark:text-slate-900 dark:dark:text-white font-semibold">صلاحيات الموظف</p>
              <p className="text-sm text-slate-500 dark:text-slate-500 dark:dark:text-white/50 mt-1">
                تقدر تختار أكثر من صلاحية لنفس الموظف.
              </p>
              {needsBranch ? (
                <p className="text-xs text-amber-700 dark:text-amber-200/90 mt-2">
      ...
      {showBranches ? (
        <div>
          ...
          <div
            className={`${ws.glassSoft} border border-slate-200 dark:border-slate-200 dark:dark:border-white/10 rounded-2xl p-4 max-h-48 overflow-y-auto`}
          >
            <div className="grid grid-cols-2 gap-3">
              {branches && branches.length > 0 ? (
                branches.map((branch) => (
                  <label
                    key={branch.id}
                    className="flex items-center gap-3 p-3 bg-slate-50 dark:bg-slate-50 dark:dark:bg-white/[0.03] hover:bg-slate-100 dark:hover:bg-slate-100 dark:dark:hover:bg-white/[0.06] rounded-2xl cursor-pointer transition-colors border border-slate-200 dark:border-slate-200 dark:dark:border-white/10"
                  >
                    <input
                      type="checkbox"
                    ...
                    <div className="flex-1">
                      <p className="text-slate-900 dark:text-slate-900 dark:dark:text-white text-sm font-medium">
                        {branch.name}
                      </p>
                      <p className="text-slate-500 dark:text-slate-500 dark:dark:text-white/45 text-xs">{branch.location}</p>
                    </div>
                  </label>
                ))
              ) : (
                <p className="col-span-2 text-center text-slate-500 dark:text-slate-500 dark:dark:text-white/45 py-4">
                  لا توجد فروع متاحة
                </p>
              )}
            </div>
        ...
        <div
          className={`${ws.glassSoft} border border-slate-200 dark:border-slate-200 dark:dark:border-white/10 rounded-2xl p-4`}
        >
          <p className="text-slate-700 dark:text-slate-700 dark:dark:text-white/70 text-sm">المدير لا يحتاج تحديد فروع.</p>
        </div>
      )}
    </div>
  );
