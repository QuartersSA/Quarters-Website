...
export default function GlassMultiSelect({
  ...
  return (
    <div className={className} dir={dir}>
      ...
      >
        <span
          className={`min-w-0 truncate ${selectedLabels.length ? "text-slate-900 dark:text-slate-900 dark:text-white" : "text-slate-500 dark:text-slate-500 dark:text-white/45"}`}
        >
          {label}
        </span>
        ...
        <span className="flex items-center gap-2 flex-shrink-0">
          {showClear ? (
            <span
              ...
              className="p-1 rounded-md hover:bg-slate-200 dark:hover:bg-slate-200 dark:hover:bg-white/10 text-slate-600 dark:text-slate-600 dark:text-white/60"
              aria-label="مسح"
              title="مسح"
            >
              <X className="w-4 h-4" />
          ...
          <ChevronDown
            className={`w-4 h-4 ${open ? "text-slate-900 dark:text-slate-900 dark:text-white" : "text-slate-600 dark:text-slate-600 dark:text-white/60"}`}
          />
        </span>
      </button>
      ...
      >
        <div className="max-h-[50vh] overflow-auto">
          <div className="px-4 py-2 border-b border-slate-200 dark:border-slate-200 dark:border-white/10 flex items-center justify-between gap-3">
            <div className="text-xs font-semibold text-slate-600 dark:text-slate-600 dark:text-white/55">
              اختيار متعدد
            </div>
            <button
              type="button"
              ...
              className="text-xs text-slate-700 dark:text-slate-700 dark:text-white/70 hover:text-slate-900 dark:hover:text-slate-900 dark:hover:text-white disabled:opacity-40"
            >
              مسح الكل
            </button>
          </div>
