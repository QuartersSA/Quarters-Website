...
export default function GlassDatePicker({
  ...
  const buttonTextClass = value ? "text-slate-900 dark:text-white" : "text-slate-400 dark:text-white/35";
  ...
  const dayPickerClassNames = useMemo(() => {
    ...
    return {
      ...
      caption_label: `${defaultClassNames.caption_label || ""} text-white/90 text-sm font-semibold`,
      ...
      chevron: `${defaultClassNames.chevron} text-slate-700 dark:text-white/70`,

      month_grid: `${defaultClassNames.month_grid} w-full border-collapse table-fixed`,
      weekdays: `${defaultClassNames.weekdays} `,
      weekday: `${defaultClassNames.weekday} p-1 text-center text-[11px] text-slate-500 dark:text-white/45 font-semibold`,
      ...
      day_button:
        ...
        "text-slate-800 dark:text-white/85 hover:bg-slate-100 dark:hover:bg-white/[0.06] focus:outline-none focus:ring-2 focus:ring-emerald-400/20 flex items-center justify-center",

      today: `${defaultClassNames.today} text-sky-700 dark:text-sky-200`,
      outside: `${defaultClassNames.outside} text-slate-400 dark:text-white/25`,
      disabled: `${defaultClassNames.disabled} text-slate-400 dark:text-white/25 line-through`,
    ...
    <div className={className} dir={dir}>
      <div className="relative">
        ...
        >
          ...
          <div
            className={`${ws.iconBox} w-9 h-9 rounded-xl bg-slate-50 dark:bg-white/[0.03] flex-shrink-0`}
            aria-hidden="true"
          >
            {showTime ? (
              <Clock className="w-4 h-4 text-slate-700 dark:text-white/65" />
            ) : (
              <CalendarDays className="w-4 h-4 text-slate-700 dark:text-white/65" />
            )}
          </div>
        </button>
        ...
        {allowClear && value ? (
          ...
          >
            <X className="w-4 h-4 text-slate-700 dark:text-white/70" />
          </button>
        ) : null}
      </div>
      ...
      >
        <div className="p-3">
          <div className="flex items-center justify-between gap-2 px-1 pb-2">
            <div className="text-sm font-semibold text-white/80">التاريخ</div>
          ...
          <div className="rounded-2xl border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-white/[0.02] p-2 overflow-hidden">
            <DayPicker
              mode="single"
              captionLayout="label"
                ...
                Chevron: (props) => {
                  ...
                  if (dir === "rtl") {
                    ...
                    if (props.orientation === "left") {
                      return (
                        <ChevronRight
                          className={`${className} w-4 h-4 text-slate-700 dark:text-white/70`}
                        />
                      );
                    }
                    return (
            ...
            <div className="mt-3 pt-3 border-t border-slate-200 dark:border-white/10">
              ...
              <div className="flex items-center gap-3">
                <div className="flex-1">
                  <label className="block text-[11px] text-slate-500 dark:text-white/45 mb-1">
                    الساعة
                  </label>
                  <select
                    value={
                ...
                <span className="text-slate-500 dark:text-white/50 text-lg font-bold mt-4">:</span>
                <div className="flex-1">
                  <label className="block text-[11px] text-slate-500 dark:text-white/45 mb-1">
                    الدقيقة
                  </label>
          ...
          <div className="pt-2 text-xs text-slate-500 dark:text-white/45">
            {showTime
              ? "* اختر التاريخ والوقت"
