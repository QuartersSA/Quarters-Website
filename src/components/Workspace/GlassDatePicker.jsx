...
export default function GlassDatePicker({
  ...
  const buttonTextClass = value ? "text-white" : "text-white/35";
  ...
  const dayPickerClassNames = useMemo(() => {
    ...
    return {
      ...
      chevron: `${defaultClassNames.chevron} text-white/70`,

      month_grid: `${defaultClassNames.month_grid} w-full border-collapse table-fixed`,
      weekdays: `${defaultClassNames.weekdays} `,
      weekday: `${defaultClassNames.weekday} p-1 text-center text-[11px] text-white/45 font-semibold`,
      ...
      day_button:
        ...
        "text-white/85 hover:bg-white/[0.06] focus:outline-none focus:ring-2 focus:ring-emerald-400/20 flex items-center justify-center",

      today: `${defaultClassNames.today} text-sky-200`,
      outside: `${defaultClassNames.outside} text-white/25`,
      disabled: `${defaultClassNames.disabled} text-white/25 line-through`,
    ...
    <div className={className} dir={dir}>
      <div className="relative">
        ...
        >
          ...
          <div
            className={`${ws.iconBox} w-9 h-9 rounded-xl bg-white/[0.03] flex-shrink-0`}
            aria-hidden="true"
          >
            {showTime ? (
              <Clock className="w-4 h-4 text-white/65" />
            ) : (
              <CalendarDays className="w-4 h-4 text-white/65" />
            )}
          </div>
        </button>
      ...
      >
        <div className="p-3">
          ...
          <div className="rounded-2xl border border-white/10 bg-white/[0.02] p-2 overflow-hidden">
            <DayPicker
              mode="single"
              captionLayout="label"
                ...
                Chevron: (props) => {
                  ...
                  if (dir === "rtl") {
                    ...
                    if (props.orientation === "left") {
                      return (
                        <ChevronRight
                          className={`${className} w-4 h-4 text-white/70`}
                        />
                      );
                    }
                    return (
          ...
          {showTime ? (
            <div className="mt-3 pt-3 border-t border-white/10">
              ...
              <div className="flex items-center gap-3">
                ...
                <span className="text-white/50 text-lg font-bold mt-4">:</span>
                <div className="flex-1">
                  <label className="block text-[11px] text-white/45 mb-1">
                    الدقيقة
                  </label>
