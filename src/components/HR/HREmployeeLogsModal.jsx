          ...
          ? fieldEntries.map(([key, v]) => {
              ...
              return (
                <div key={key} className="text-sm text-slate-700 dark:text-white/75">
                  <span className="font-semibold text-slate-800 dark:text-white/85">{label}</span>
...
export function HREmployeeLogsModal({ isOpen, employee, onClose }) {
  ...
  if (isLoadingLogs) {
    content = (
      <div className="text-center text-slate-600 dark:text-white/60 py-12">جاري تحميل السجل…</div>
    );
  } else if (logsError) {
    content = (
    ...
    content = (
      <div className="text-center text-slate-600 dark:text-white/60 py-12">
      ...
      <div className="space-y-3">
        {logsList.map((log) => {
          ...
          return (
            <div
              key={log.id}
              className={`${ws.glass} ${ws.card} p-4 border border-slate-200 dark:border-white/10`}
            >
              ...
              {summary ? (
                <div className="mt-2 text-slate-700 dark:text-white/75 text-sm">{summary}</div>
              ) : null}
        ...
        <div className="flex items-center justify-between p-5 border-b border-slate-200 dark:border-white/10">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-sm text-slate-600 dark:text-white/55">آخر 200 حركة</div>
            </div>
          </div>
