      ...
      >
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {group.isLegacy ? (
                ...
                <span className="text-[10px] text-emerald-300 px-2 py-0.5 rounded-full bg-emerald-400/10 border border-emerald-400/30 font-mono">
        ...
        <div className="flex items-center gap-3 flex-shrink-0">
          <div className="text-left">
            <div className="text-xs text-white/40">إجمالي الكمية</div>
            <div className="text-white font-bold" dir="ltr">
              {formatNumber(group.totalQty)}
            </div>
          </div>
          <div className={`${ws.iconBox} w-9 h-9 text-white/40`}>
            {expanded ? (
      ...
      {expanded ? (
        <div className="mt-4 space-y-2 pt-4 border-t border-white/10">
          {group.items.map((item) => (
            <div
              key={item.id}
              className="flex items-center justify-between gap-3 px-3 py-2 rounded-xl bg-white/[0.04] border border-white/10"
            >
...
export function ReceiptsList({ groups, isLoading, error }) {
  if (isLoading) {
    return (
      <div className={`${sectionCard} p-12 text-center text-white/55`}>
  ...
  if (error) {
    return (
      <div className={`${sectionCard} p-12 text-center text-red-300`}>
  ...
  if (!Array.isArray(groups) || groups.length === 0) {
    return (
      <div className={`${sectionCard} p-12 text-center text-white/50`}>
